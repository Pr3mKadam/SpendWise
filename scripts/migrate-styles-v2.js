...
function _debug(msg) { process.stderr.write(msg + '\n'); }
...
function normalizePropValue(p) {
  let s = p.trim();
  while (s.startsWith('/*') && s.includes('*/')) { s = s.substring(s.indexOf('*/') + 2).trim(); }
  return s;
}
...
function parseStyleLine(line) {
  ...
  while (val.startsWith('/*') && val.includes('*/')) { val = val.substring(val.indexOf('*/') + 2).trim(); }
...
function lineToTwClasses(line) {
  ...
  if (prop === 'borderRadius') {
    ...
    const twMap = { '4px': 'rounded', '6px': 'rounded-md', '8px': 'rounded-lg', '10px': 'rounded-lg', '12px': 'rounded-xl', '16px': 'rounded-2xl', '20px': 'rounded-3xl', '24px': 'rounded-3xl', '50%': 'rounded-full', '999px': 'rounded-full' };
  ...
  if (prop === 'height' || prop === 'minHeight' || prop === 'maxHeight') {
    const v = val.replace(/'/g, '');
    if (v === '1px') return [prop === 'height' ? 'h-px' : prop === 'minHeight' ? 'min-h-px' : 'max-h-px'];
    if (v === '100%') return [prop === 'height' ? 'h-full' : prop === 'minHeight' ? 'min-h-full' : 'max-h-full'];
    const tw = pxToTw(v);
    if (tw) return [prop === 'height' ? `h-${tw}` : prop === 'minHeight' ? `min-h-${tw}` : `max-h-${tw}`];
    const num = parseInt(v);
    if (!isNaN(num) && v === String(num)) {
      if (num % 4 === 0) return [prop === 'height' ? `h-${num/4}` : prop === 'minHeight' ? `min-h-${num/4}` : `max-h-${num/4}`];
      return [prop === 'height' ? `h-[${num}px]` : prop === 'minHeight' ? `min-h-[${num}px]` : `max-h-[${num}px]`];
    }
    return null;
  }
  ...
  if (prop === 'width' || prop === 'minWidth' || prop === 'maxWidth') {
    const v = val.replace(/'/g, '');
    if (v === '1px') return [prop === 'width' ? 'w-px' : prop === 'minWidth' ? 'min-w-px' : 'max-w-px'];
    if (v === '100%') return [prop === 'width' ? 'w-full' : prop === 'minWidth' ? 'min-w-full' : 'max-w-full'];
    const tw = pxToTw(v);
    if (tw) return [prop === 'width' ? `w-${tw}` : prop === 'minWidth' ? `min-w-${tw}` : `max-w-${tw}`];
    const num = parseInt(v);
    if (!isNaN(num) && v === String(num)) {
      if (num % 4 === 0) return [prop === 'width' ? `w-${num/4}` : prop === 'minWidth' ? `min-w-${num/4}` : `max-w-${num/4}`];
      return [prop === 'width' ? `w-[${num}px]` : prop === 'minWidth' ? `min-w-[${num}px]` : `max-w-[${num}px]`];
    }
    return null;
  }
  ...
  if (prop === 'borderBottom' || prop === 'borderTop' || prop === 'borderLeft' || prop === 'borderRight') {
    const v = val.replace(/'/g, '');
    if (v === 'none') return [`${prop === 'borderBottom' ? 'border-b' : prop === 'borderTop' ? 'border-t' : prop === 'borderLeft' ? 'border-l' : 'border-r'}-0`];
    return null;
  }
...
function processFile(filePath) {
  ...
  const updated = content.replace(/style=\{(\{[^}]+\})\}/g, (match, styleContent) => {
    ...
    const lines = inner.split(/,(?=(?:[^']*'[^']*')*[^']*$)/g).map(s => s.trim()).filter(Boolean);
