...
test.describe('Step 3, 4, 5: Dashboard & Transactions', () => {
  ...
  test('3.1 & 3.2 - Dashboard elements render correctly', async ({ page }) => {
    ...
    await expect(page.getByText(/Recent/i).or(page.getByText(/Transaction History/i)).first()).toBeVisible();
  });
  ...
  test('5.1 to 5.6 - Transaction History and Filtering', async ({ page }) => {
    ...
    await page.getByRole('tab', { name: 'Transactions' }).or(page.getByRole('button', { name: 'Transactions' })).first().click();
