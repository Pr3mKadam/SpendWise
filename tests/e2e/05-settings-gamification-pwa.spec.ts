...
test.describe('Step 16-22: Gamification, Profile, Privacy & Settings', () => {
  ...
  test('16.1 to 16.5 - Gamification & XP', async ({ page }) => {
    ...
    await expect(page.getByText(/Quests/i).filter({ visible: true }).first()).toBeVisible();
  });
  ...
  test('18.1 to 18.5 - Parental Controls', async ({ page }) => {
    ...
    await expect(page.getByText(/Parental/i).filter({ visible: true }).first()).toBeVisible();
    ...
    if (await setupPinBtn.isVisible()) {
      ...
      // Dashboard unlocks
      await expect(page.getByText(/Monthly Limit/i).filter({ visible: true }).first()).toBeVisible();
    }
  });
