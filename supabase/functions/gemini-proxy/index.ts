...
function validateContents(body: unknown): { contents: unknown[]; generationConfig?: unknown } {
  ...
  if (rawLength > MAX_CONTENT_LENGTH) {
    throw new Error(`Payload too large. Maximum content length is ${MAX_CONTENT_LENGTH} characters.`);
  }
  return {
...
serve(async (req) => {
  ...
  try {
    ...
    if (!geminiResponse.ok) {
      ...
      const safeError = typeof errorText === 'string' && errorText.length > 200
        ? errorText.slice(0, 200)
        : errorText || 'Gemini API error';
      return new Response(
        JSON.stringify({ error: `Gemini API Error: ${safeError}` }),
    ...
    const status = message.includes('body') || message.includes('Missing') || message.includes('too large')
      ? 400
      : 500;
    return new Response(
