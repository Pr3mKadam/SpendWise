...
function validateBody(raw: unknown): SetuPayload {
  ...
  if (!['create-consent', 'check-consent', 'fetch-statements'].includes(obj.action)) {
    throw new Error(`Unknown action: "${obj.action}". Valid actions: create-consent, check-consent, fetch-statements`);
  }

  switch (obj.action) {
    case 'create-consent': {
      if (typeof obj.environment !== 'string' || !['sandbox', 'production'].includes(obj.environment)) {
      ...
      return { action: 'create-consent', environment: obj.environment as 'sandbox' | 'production', mobileNumber: obj.mobileNumber };
    }
    case 'check-consent': {
  ...
  try {
    ...
    switch (payload.action) {
      case 'create-consent': {
        ...
        if (!setuResp.ok) {
          return new Response(
            JSON.stringify({ error: data?.message || "Setu AA API error" }),
            { status: setuResp.status, headers: { ...corsHeaders, "Content-Type": "application/json" } }
          );
        }

        return new Response(
          JSON.stringify({ id: data.id, url: data.url, status: "PENDING" }),
      ...
      case 'fetch-statements': {
        ...
        return new Response(
          JSON.stringify({ transactions: data.transactions || [] }),
          { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } }
        );
      }
    }
    ...
    const status = message.includes('body') || message.includes('Missing') || message.includes('Unknown')
      ? 400
      : 500;
    return new Response(
