...
export async function generateTotpCode(secret: ArrayBuffer, timestamp = Date.now()): Promise<string> {
...
export function parseOtpauthUrl(url: string): { secret: string; issuer: string; account: string } | null {
  try {
    const u = new URL(url);
...
export function prepareRecoveryCodesForStorage(codes: RecoveryCode[]): Promise<StoredRecoveryCode[]> {
  return Promise.all(
