  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {notif.link && (
          <span className="mt-1 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity" style={{ fontFamily: 'var(--font-inter)', fontSize: '11px', fontWeight: 600, color: 'var(--teal)' }}>
            <ExternalLink size={10} /> Go to {notif.link.charAt(0).toUpperCase() + notif.link.slice(1)}
          </span>
        )}
      </div>
      {!notif.read && (
        <span className="mt-1.5 h-2 w-2 rounded-full shrink-0" style={{ background: 'var(--teal)' }} />
      )}
    </div>
  );
}
...
export default function NotificationCenter({ notifications, unreadCount, isOpen, onClose, onMarkRead, onMarkAllRead, onNavigate }: NotificationCenterProps) {
  ...
  useEffect(() => {
    if (!isOpen) return;
    const fn = (e: MouseEvent) => { if (panelRef.current && !panelRef.current.contains(e.target as Node)) onClose(); };
  ...
  const grouped = { unread: notifications.filter(n => !n.read), read: notifications.filter(n => n.read) };

  return (
    <>
      {/* Backdrop */}
      <div className="fixed inset-0 z-[60]" style={{ background: 'rgba(0,0,0,0.15)' }} onClick={onClose} />
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 shrink-0" style={{ borderBottom: '1.5px solid #f0f2f5' }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 style={{ fontFamily: 'var(--font-manrope)', fontSize: '14px', fontWeight: 700, color: 'var(--text-primary)' }}>Notifications</h3>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {notifications.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-14 text-center">
              ...
              <p style={{ fontFamily: 'var(--font-inter)', fontSize: '12px', color: 'var(--text-dim)', marginTop: '4px' }}>You're all caught up!</p>
            </div>
          ) : (
            <>
              {grouped.unread.length > 0 && (
                <>
                  <div className="sticky top-0 px-4 py-2" style={{ background: 'var(--surface-card)' }}>
                    <p style={{ fontFamily: 'var(--font-inter)', fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--text-muted)' }}>
              ...
              {grouped.read.length > 0 && (
                <>
                  ...
                  {grouped.read.map(n => <NotifRow key={n.id} notif={n} onMarkRead={onMarkRead} onNavigate={onNavigate} onClose={onClose} />)}
                </>
              )}
            </>
          )}
