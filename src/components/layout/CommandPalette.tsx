...
export default function CommandPalette({
  ...
  const VIEWS = [
    {
      ...
      keywords: 'home start overview stats',
    },
    {
      id: 'budget',
      label: 'Budgets',
      icon: <DollarSign size={16} />,
      keywords: 'limit allowance rings',
    },
    {
      id: 'analytics',
      label: 'Analytics',
    ...
    },
    {
      ...
      keywords: 'transactions list export report pdf csv',
    },
    {
      id: 'goals',
      label: 'Savings Goals',
      icon: <Target size={16} />,
      keywords: 'vault piggy stash target',
    },
    {
      id: 'shared',
      label: 'Shared Wallets',
      icon: <Wallet size={16} />,
      keywords: 'group roommates split expense',
    },
    {
      id: 'sync',
      label: 'Bank Sync',
      icon: <RefreshCw size={16} />,
      keywords: 'upi phonepe gpay offline csv',
    },
    {
      id: 'profile',
      label: 'Profile',
      icon: <User size={16} />,
      keywords: 'account name pin parental',
    },
    {
      id: 'subscriptions',
      label: 'Subscriptions',
      icon: <Activity size={16} />,
      keywords: 'recurring monthly fixed netflix',
    },
    {
      id: 'portfolio',
      label: 'Portfolio',
      icon: <PiggyBank size={16} />,
      keywords: 'net worth assets crypto stocks',
    },
  ];

  // Filter Views
  const lowerQuery = query.toLowerCase();
  const matchedViews = query
    ? VIEWS.filter(
        v => v.label.toLowerCase().includes(lowerQuery) || v.keywords.includes(lowerQuery)
      )
    : VIEWS;
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowDown') {
        ...
        e.preventDefault();
        setSelectedIndex(
          prev => (prev - 1 + Math.max(totalResults, 1)) % Math.max(totalResults, 1)
        );
      } else if (e.key === 'Enter') {
        e.preventDefault();
  ...
  return (
    <Portal>
      <div
        className="fixed inset-0 z-[100] flex items-start justify-center pt-[15vh] sm:pt-[20vh] px-4 animate-fade-in-up"
        ...
        >
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div
                          ...
                          style={{
                            background:
                              tx.type === 'credit' ? 'var(--green-dim)' : 'var(--surface-card)',
                        ...
                        <div>
                          <p className="font-inter font-semibold text-[13px] text-[var(--text-primary)] leading-tight">
                            {tx.merchant}
                          </p>
                          <p className="font-inter text-[length:var(--fs-caption)] text-[var(--text-muted)]">
                      ...
                      <span
                        ...
                        style={{
                          color: tx.type === 'credit' ? 'var(--green)' : 'var(--text-primary)',
                        }}
                      >
                        {tx.type === 'credit' ? '+' : '-'}
                        {currency}
          ...
          <div className="px-4 py-2 border-t border-[var(--border)] bg-[var(--surface-input)] flex items-center gap-4 text-[length:var(--fs-caption)] text-[var(--text-muted)] font-inter">
            <span className="flex items-center gap-1">
              <kbd className="bg-[var(--surface-card)] px-1.5 py-0.5 rounded border border-[var(--border)] shadow-sm">
                ↑
              </kbd>{' '}
              <kbd className="bg-[var(--surface-card)] px-1.5 py-0.5 rounded border border-[var(--border)] shadow-sm">
                ↓
            ...
            <span className="flex items-center gap-1">
              <kbd className="bg-[var(--surface-card)] px-1.5 py-0.5 rounded border border-[var(--border)] shadow-sm font-sans">
                ↵
              </kbd>{' '}
              to select
            </span>
