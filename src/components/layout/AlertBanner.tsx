...
export default function AlertBanner({ alerts, onDismiss, onDismissAll }: AlertBannerProps) {
  ...
  const sorted = [...alerts].sort(
    (a, b) =>
      ({ danger: 0, warning: 1, info: 2 })[a.severity] -
      { danger: 0, warning: 1, info: 2 }[b.severity]
  );
        ...
        <div className="flex items-center gap-2">
          ...
          {danger > 0 && (
            <span
              ...
              style={{ background: 'var(--red)', fontFamily: 'var(--font-inter)' }}
            >
              {danger} critical
            </span>
          )}
          ...
          {alerts.length > 1 && (
            ...
            >
              {expanded ? (
                ...
                <>
                  <ChevronDown size={12} /> +{alerts.length - 1} more
                </>
              )}
            </button>
          )}
