      ...
      <div className="flex items-center gap-3 mb-5">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p style={{ fontFamily: 'var(--font-inter)', fontSize: '12px', color: 'var(--text-muted)' }}>
            {currency}{totalMonthly.toLocaleString()}/mo across {subscriptions.length} subscriptions
          </p>
        </div>
        {/* Nav */}
        <div className="flex items-center gap-1 shrink-0">
          <button onClick={prev} className="w-7 h-7 rounded-lg flex items-center justify-center hover:bg-[var(--surface-input)] transition-colors border-none cursor-pointer" style={{ background: 'transparent' }}>
            <ChevronLeft size={15} style={{ color: 'var(--text-muted)' }} />
          </button>
          <span className="text-sm font-bold px-2" style={{ color: 'var(--text-primary)', fontFamily: 'var(--font-inter)', minWidth: 110, textAlign: 'center' }}>
            {MONTH_NAMES[month]} {year}
          </span>
          <button onClick={next} className="w-7 h-7 rounded-lg flex items-center justify-center hover:bg-[var(--surface-input)] transition-colors border-none cursor-pointer" style={{ background: 'transparent' }}>
        ...
        {cells.map((day, i) => {
          ...
          return (
            <div
              ...
              style={{
                background: hasBills ? 'rgba(59,130,246,0.06)' : isToday ? 'var(--teal-dim)' : 'transparent',
                border: isToday ? '1.5px solid var(--teal)' : hasBills ? '1px solid rgba(59,130,246,0.2)' : '1px solid transparent',
              }}
            >
              <span
              ...
              {bills.slice(0, 2).map((sub, bi) => (
                <div
                  ...
                  style={{ background: sub.color ? `${sub.color}25` : '#3b82f620', color: sub.color ?? '#3b82f6' }}
              ...
              {bills.length > 2 && (
                <div className="text-[8px] font-bold" style={{ color: '#3b82f6' }}>+{bills.length - 2}</div>
              )}
            </div>
          );
        })}
              ...
              .map(sub => (
                <div key={sub.id} className="flex items-center justify-between text-[12px]">
                  <div className="flex items-center gap-2">
                    <span>{sub.emoji ?? '💳'}</span>
                    <span style={{ color: 'var(--text-primary)', fontFamily: 'var(--font-inter)', fontWeight: 500 }}>{sub.name}</span>
                  </div>
                  <div className="flex items-center gap-3 shrink-0">
                    <span style={{ color: 'var(--text-muted)' }}>Day {sub.billingDay ?? 1}</span>
                    <span className="font-bold" style={{ color: '#3b82f6', fontFamily: 'var(--font-manrope)' }}>{currency}{sub.amount}</span>
                  </div>
                </div>
              ))}
          </div>
