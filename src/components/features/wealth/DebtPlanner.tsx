...
export default function DebtPlanner({ liabilities, currency, monthlyExtra = 0, userRole }: DebtPlannerProps) {
  ...
  const interestDifference = Math.abs((comparison?.avalanche.totalInterest || 0) - (comparison?.snowball.totalInterest || 0));
  const timeDifference = Math.abs((comparison?.avalanche.months || 0) - (comparison?.snowball.months || 0));
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            <h4 className="font-manrope font-bold text-sm text-[var(--text-primary)] mb-1">
              {userRole === 'student' ? 'Student Financial Advisor' : userRole === 'business' ? 'Business Capital Advisor' : 'Wealth Strategy Advisor'}
            </h4>
            <p className="text-xs text-[var(--text-secondary)] leading-relaxed">
              {userRole === 'student' 
              ...
              {extraPayment < 2000 && " Small tip: Increasing your extra payment by just 1,000 could cut your debt timeline by nearly 15%."}
            </p>
          </div>
        </div>
      </div>
