...
export function GoalCard({
  ...
  const [roundUpsEnabled, setRoundUpsEnabled] = useState(() => {
    try { return localStorage.getItem(ROUNDUP_KEY(goal.id)) === 'true'; } catch { return false; }
  });
  ...
  useEffect(() => {
    ...
    const fired: number[] = (() => {
      try { return JSON.parse(localStorage.getItem(MILESTONE_KEY(goal.id)) || '[]'); } catch { return []; }
    })();
        ...
        {!isAchieved && (
          <div className="mt-2 flex items-center gap-2 flex-wrap">
            ...
            {monthsToGo !== null && !isAchieved && (
              ...
              >
                <Target size={9} />
                ~{monthsToGo} {monthsToGo === 1 ? 'month' : 'months'} to go
              </span>
            )}
          </div>
        )}
