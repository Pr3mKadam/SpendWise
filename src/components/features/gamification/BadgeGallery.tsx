...
export function BadgeGallery({ transactions, streak, level, goals, currency = '₹' }: Props) {
  const badges = useMemo(() => computeBadges({ transactions, streak, level, goals }), [transactions, streak, level, goals, currency]);
  ...
  return (
    <div className="card px-4 sm:px-6 py-5">
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          <h3 style={{ fontFamily: 'var(--font-manrope)', fontSize: '18px', fontWeight: 700, color: 'var(--text-primary)' }}>
            Achievement Gallery
          </h3>
        ...
        <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl"
          ...
          <span className="text-xs font-bold" style={{ color: 'var(--teal)', fontFamily: 'var(--font-inter)' }}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3">
            {locked.map(badge => (
              ...
              >
                ...
                {/* Hover tooltip */}
                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2.5 py-1.5 rounded-xl shadow-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-20 w-40 text-center"
                  style={{ background: 'var(--surface-card)', border: '1px solid var(--border)', fontSize: '10px', fontFamily: 'var(--font-inter)', fontWeight: 600, color: 'var(--text-secondary)', lineHeight: 1.4 }}>
                  🔓 {badge.criteria}
                </div>
              </div>
            ))}
