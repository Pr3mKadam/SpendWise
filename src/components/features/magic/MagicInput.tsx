...
export default function MagicInput({ onAddTransaction, currency = '$' }: MagicInputProps) {
  ...
  const pc = store.parentalState;
  const transactions = store.transactions;

  const getParentalCheck = (amount: number, cat: string) => {
    if (!pc.isTeenMode) return { allowed: true };
    if (pc.blockAddTransactions) return { allowed: false, reason: 'Adding transactions is currently blocked by your parent.' };
    if (pc.restrictedCategories.includes(cat as Category)) return { allowed: false, reason: `Transactions in "${cat}" require parent approval.` };
    
    if (pc.monthlyLimit !== null && type === 'debit') {
      const currentMonthStr = new Date().toISOString().substring(0, 7);
      const monthlySpent = transactions
        .filter(t => t.type === 'debit' && t.date.startsWith(currentMonthStr))
        .reduce((acc, t) => acc + t.amount, 0);
      
      if (monthlySpent + amount > pc.monthlyLimit) {
        return { allowed: false, reason: 'Monthly spending limit exceeded. Requires parent approval.' };
      }
    }
    return { allowed: true };
  };
  ...
  const handleSubmit = () => {
    ...
    const check = getParentalCheck(amount, category);
    if (!check.allowed) {
