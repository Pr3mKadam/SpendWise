...
function PatternCard({ pattern, currency }: { pattern: RecurringPattern; currency: string }) {
  ...
  return (
    <div
      className="group relative overflow-hidden rounded-xl p-4 transition-all duration-200 card card-hover"
    >
      {/* Top accent */}
      <div
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p style={{ fontFamily: 'var(--font-inter)', fontSize: '12px', color: 'var(--text-muted)', marginTop: '2px' }}>
            ...
            {' · '}{pattern.occurrences}× detected · {currency}{pattern.totalSpent.toFixed(0)} total
          </p>
        </div>

        {/* Amount */}
        <div className="flex-shrink-0 text-right">
          <p style={{ fontFamily: 'var(--font-manrope)', fontSize: '15px', fontWeight: 700, color: 'var(--text-primary)' }}>
            ~{currency}{pattern.avgAmount.toFixed(2)}
          </p>
          <p style={{ fontFamily: 'var(--font-inter)', fontSize: '11px', color: 'var(--text-muted)' }}>avg / charge</p>
        </div>
      </div>

      {/* Next expected */}
      <div className="mt-3 flex items-center justify-between pt-2.5" style={{ borderTop: '1px solid #f0f2f5' }}>
        <div className="flex items-center gap-1.5">
          <Calendar size={12} style={{ color: 'var(--text-muted)' }} />
          <span style={{ fontFamily: 'var(--font-inter)', fontSize: '12px', color: 'var(--text-muted)' }}>
            Next: <span style={{ fontWeight: 600, color: 'var(--text-primary)' }}>{formatDate(pattern.nextExpected)}</span>
          </span>
        </div>
        <span
          className="flex items-center gap-1 rounded-full px-2 py-0.5"
          style={{ fontFamily: 'var(--font-inter)', fontSize: '11px', fontWeight: 600, background: isUrgent ? 'var(--amber-dim)' : '#f5f7fa', color: isUrgent ? 'var(--amber)' : 'var(--text-muted)' }}
        >
          <Clock size={10} />
          {days === 0 ? 'Today' : `${days}d`}
        </span>
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center py-12 text-center">
      <div className="mb-3 flex h-14 w-14 items-center justify-center rounded-2xl" style={{ background: '#f5f7fa' }}>
        <RefreshCw size={24} style={{ color: 'var(--text-muted)' }} />
      </div>
      <p style={{ fontFamily: 'var(--font-inter)', fontSize: '14px', fontWeight: 500, color: 'var(--text-muted)' }}>No recurring patterns found</p>
      <p style={{ fontFamily: 'var(--font-inter)', fontSize: '12px', color: 'var(--text-dim)', marginTop: '4px' }}>
      ...
      {[
        ...
        <div key={s.label} className="card px-4 py-3">
          <div className="mb-2 flex items-center gap-1.5">
            ...
            <span style={{ fontFamily: 'var(--font-inter)', fontSize: '10px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--text-muted)' }}>{s.label}</span>
          </div>
          <p style={{ fontFamily: 'var(--font-manrope)', fontSize: '16px', fontWeight: 700, color: colors[i] }}>{s.value}</p>
        </div>
      ))}
    </div>
  );
