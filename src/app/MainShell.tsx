...
const ParentalPinGate = lazy(() =>
  import('@/features/parental/components/ParentalControlGate').then(m => ({
    default: m.ParentalPinGate,
  }))
);
const KidModeBanner = lazy(() =>
  import('@/features/parental/components/ParentalControlGate').then(m => ({
    default: m.KidModeBanner,
  }))
);
const QuickAddModal = lazy(() =>
  import('@/components/layout/QuickAddModal').then(m => ({ default: m.QuickAddModal }))
);
const FeedbackModal = lazy(() =>
  import('@/components/layout/FeedbackModal').then(m => ({ default: m.FeedbackModal }))
);
...
export function MainShell({
  ...
  const { isOffline } = useAppEnvironment();
  ...
  const handlePDFReport = useCallback(async () => {
    ...
    transactions,
    financeState.monthlyStats,
    appState.budgetState.budgetStats,
    appState.goalsState.goals,
    currency,
  ]);
    ...
    <div className="flex min-h-screen flex-col" style={{ background: 'var(--bg)' }}>
      ...
      <Suspense fallback={null}>
        {pcSettings.enabled && !pcSettings.sessionUnlocked && !isKidMode && (
          <ParentalPinGate
            onContinueAsKid={() => store.updateParentalSettings({ isTeenMode: true })}
          />
        )}

        {isKidMode && <KidModeBanner onParentLogin={() => setShowParentalGate(true)} />}
