...
export async function fetchEquityPrice(symbol: string): Promise<PriceResult | null> {
  try {
    ...
    const data = await res.json();
    if (data.price) return { symbol, price: data.price, currency: 'INR', updatedAt: new Date().toISOString() };
    return null;
  } catch { return null; }
}

export async function refreshAllPrices(assets: Array<{ id: string; symbol: string; type: string }>): Promise<Record<string, number>> {
