...
export const handlePortfolioAdjust: IntentHandler = ({ command }) => {
  ...
  if (amount === undefined)
    return { success: false, message: `What should I set the value of ${target} to?` };
  ...
  return {
    success: true,
    message: `✅ Updated ${asset.name} value to ${formatCurrency(amount)}.`,
    undoable: true,
  };
};
...
export const handleGoalUpdate: IntentHandler = ({ command }) => {
  ...
  const goal = store.assets.find(
    a => a.type === 'other' && a.name.toLowerCase().includes(name.toLowerCase())
  );
  ...
  if (amount && amount > 0) {
    ...
    return {
      success: true,
      message: `✅ Added ${formatCurrency(amount)} to ${goal.name}.`,
      undoable: true,
    };
  } else if (targetAmount && targetAmount > 0) {
    return {
      success: false,
      message: `I can add money to the goal, but changing the target isn't supported yet.`,
    };
  }
