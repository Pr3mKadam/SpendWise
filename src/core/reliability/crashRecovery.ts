...
export function recordCrash(): void {
  try {
    ...
    logger.system.error(`Crash recorded (${newCount} in window)`, { crashCount: newCount, withinWindow });
...
export function registerCrashRecovery(): void {
  ...
  if (isInRecoveryMode()) {
    performRecovery().then(recovered => {
      if (recovered) {
        captureError(new Error('App recovered from crash loop'), { recoveryAction: getRecoveryAction() });
      }
    });
  }
}
