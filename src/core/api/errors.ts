...
export function normalizeError(err: unknown): NormalizedError {
  if (err instanceof ApiError) {
    return { code: err.code, message: err.message, status: err.status, details: err.details, retryable: err.retryable };
  }
  ...
  if (err instanceof ZodError) {
    return {
      ...
      details: err.issues.map(e => ({ path: e.path.map(p => String(p)).join('.'), message: e.message })),
      retryable: false,
    };
  }
  ...
  if (err instanceof TypeError && err.message === 'Failed to fetch') {
    return { code: 'NETWORK_ERROR', message: 'Network request failed. Check your connection.', retryable: true };
  }
...
function parseErrorBody(body: string): { message: string; details?: unknown } {
  try {
    ...
    const msg = parsed.error?.description || parsed.error?.message || parsed.msg || parsed.error_description || parsed.message || body;
