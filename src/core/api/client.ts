...
export class ApiClient {
  ...
  async request<T>(config: RequestConfig, responseSchema?: ZodType<T>): Promise<T> {
    if (!navigator.onLine) {
      throw new ApiError({
        code: 'NETWORK_ERROR',
        message: 'No internet connection. Operation queued.',
        retryable: true,
      });
    }
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (!response.ok) {
          ...
          logger.system.warn(`API ${config.method} ${config.url} failed`, {
  ...
  async post<TInput, TOutput>(
    ...
    return this.request<TOutput>(
      { method: 'POST', url, body: body as Record<string, unknown>, ...config },
      schema
    );
  }

  async put<TInput, TOutput>(
    url: string,
    body?: TInput,
    config?: Partial<RequestConfig>,
    schema?: ZodType<TOutput>
    ...
    return this.request<TOutput>(
      { method: 'PUT', url, body: body as Record<string, unknown>, ...config },
      schema
    );
  }

  async patch<TInput, TOutput>(
    url: string,
    body?: TInput,
    config?: Partial<RequestConfig>,
    schema?: ZodType<TOutput>
    ...
    return this.request<TOutput>(
      { method: 'PATCH', url, body: body as Record<string, unknown>, ...config },
      schema
    );
  }
