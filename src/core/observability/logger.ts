...
function createLogger(context: LogContext) {
  const entry = (level: LogLevel, message: string, data?: Record<string, unknown>, error?: unknown): LogEntry => ({
    ...
    data,
    error: error
      ? { name: (error as Error).name || 'Error', message: (error as Error).message || String(error), stack: (error as Error).stack }
      : undefined,
  });
  ...
  return {
    ...
    warn: (msg: string, data?: Record<string, unknown>, error?: unknown) => log('WARN', msg, data, error),
    error: (msg: string, data?: Record<string, unknown>, error?: unknown) => log('ERROR', msg, data, error),
  };
}
