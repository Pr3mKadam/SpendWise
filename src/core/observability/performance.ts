...
function reportVital(name: string, value: number): void {
  ...
  if (rating === 'poor') {
    captureMessage(`Poor Web Vital: ${name}`, 'warning', { value: rounded, threshold: VITAL_THRESHOLDS[name]?.poor });
  }
}
...
export function reportMemoryUsage(): void {
  if ('memory' in performance) {
    ...
    logger.perf.debug('Memory usage', { usedMB, limitMB, pct: Math.round((usedMB / limitMB) * 100) });
  }
}
