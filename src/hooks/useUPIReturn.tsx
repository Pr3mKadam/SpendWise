...
function cleanURLParams() {
  ...
  ['upi_status','upi_tr','upi_pa','upi_pn','upi_am','upi_tn','Status','txnId','txnRef','responseCode','ApprovalRefNo']
...
async function addUPITransaction(
  ...
  try {
    ...
    toast.success(`✅ ₹${result.amount.toFixed(0)} to ${result.pn || 'Merchant'} added to SpendWise!`, {
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        ...
        <button
          ...
          style={{
            ...
            color: '#64748b', fontWeight: 700, cursor: 'pointer', fontSize: 12,
          }}
        >
          ❌ Failed
        </button>
...
function showUPIConfirmToast(
  ...
  confirmToastId = toast(
    (t) => (
      <UPIConfirmToast t={t} pending={pending} onTransactionAdded={onTransactionAdded} />
    ),
...
export function useUPIReturn({ onTransactionAdded, onPaymentDetected }: UseUPIReturnOptions) {
  ...
  const handleUPIReturn = useCallback(async (result: UPIPaymentResult) => {
    ...
    try {
      ...
      if (enriched.status === 'SUCCESS' || enriched.status === 'SUBMITTED') {
        ...
        if (enriched.amount > 0) {
          ...
          toast.success('💸 UPI payment returned. Verify your transaction history.', { duration: 5000 });
        }
  ...
  useEffect(() => {
    ...
    const onVisibilityChange = () => {
      ...
      if (urlResult) { handleUPIReturn(urlResult); return; }
