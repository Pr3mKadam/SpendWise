...
const MERCHANT_CATEGORY_MAP: Record<string, DefaultCategory> = {
  // Food & Dining
  swiggy: "Food",       zomato: "Food",      mcdonalds: "Food",
  dominos: "Food",      kfc: "Food",          pizzahut: "Food",
  starbucks: "Food",    dunkin: "Food",       subway: "Food",
  blinkit: "Food",      bigbasket: "Food",    grofers: "Food",
  instamart: "Food",    zepto: "Food",        fresh: "Food",

  // Transport
  uber: "Transport",    ola: "Transport",     rapido: "Transport",
  irctc: "Transport",   railway: "Transport", redbus: "Transport",
  makemytrip: "Transport", goibibo: "Transport", petrol: "Transport",
  bpcl: "Transport",    hpcl: "Transport",    indianoil: "Transport",
  metro: "Transport",   fastag: "Transport",  nhai: "Transport",

  // Shopping
  amazon: "Shopping",   flipkart: "Shopping", myntra: "Shopping",
  ajio: "Shopping",     meesho: "Shopping",   nykaa: "Shopping",
  snapdeal: "Shopping", tatacliq: "Shopping", reliance: "Shopping",
  dmart: "Shopping",    bigbazar: "Shopping",

  // Subscriptions
  netflix: "Subscriptions",    spotify: "Subscriptions",
  hotstar: "Subscriptions",    disney: "Subscriptions",
  youtube: "Subscriptions",    prime: "Subscriptions",
  zee5: "Subscriptions",       sonyliv: "Subscriptions",
  jiocinema: "Subscriptions",  mangotv: "Subscriptions",
  linkedin: "Subscriptions",   notion: "Subscriptions",
  adobe: "Subscriptions",      microsoft: "Subscriptions",

  // Utilities
  electricity: "Utilities",    bses: "Utilities",
  tata: "Utilities",           airtel: "Utilities",
  jio: "Utilities",            vi: "Utilities",
  vodafone: "Utilities",       mahanagar: "Utilities",
  gas: "Utilities",            water: "Utilities",
  internet: "Utilities",       broadband: "Utilities",

  // Health
  apollo: "Health",            medanta: "Health",
  fortis: "Health",            "1mg": "Health",
  pharmeasy: "Health",         netmeds: "Health",
  hospital: "Health",          clinic: "Health",
  pharmacy: "Health",          chemist: "Health",
  doctor: "Health",            lab: "Health",

  // Education
  udemy: "Education",          coursera: "Education",
  unacademy: "Education",      byju: "Education",
  vedantu: "Education",        tuition: "Education",
  school: "Education",         college: "Education",
  fee: "Education",

  // Entertainment
  bookmyshow: "Entertainment", pvr: "Entertainment",
  inox: "Entertainment",       cinemas: "Entertainment",
  gaming: "Entertainment",     steam: "Entertainment",
  playstation: "Entertainment",

  // Travel
  airasia: "Travel",           indigo: "Travel",
  spicejet: "Travel",          vistara: "Travel",
  airindia: "Travel",          hotel: "Travel",
  oyo: "Travel",               airbnb: "Travel",
  yatra: "Travel",

  // Business / Income
  salary: "Income",            stipend: "Income",
  freelance: "Income",         refund: "Income",
  cashback: "Income",          reward: "Income",
...
function learnMerchant(merchant: string, category: DefaultCategory) {
  ...
  const memory = { ...(store.merchantMemory ?? {}), [merchant.toLowerCase()]: { merchant, category } };
...
const UPI_PATTERNS = [
  ...
  {
    pattern: /Rs\.?\s*([\d,]+\.?\d*)\s+(debited|credited)\s+.*?to\s+UPI[-\s]([A-Z0-9\s]+)[-\s](\S+@\S+)/i,
    extract: (m: RegExpMatchArray) => ({
  ...
  {
    pattern: /A\/c\s+\S+\s+(debited|credited)\s+by\s+INR\s+([\d,]+\.?\d*)\s+on\s+([\d\/-]+)\.?\s+(?:.*?Ref[:\s]+(\d+))?/i,
    extract: (m: RegExpMatchArray) => ({
...
export function parseUPIString(text: string): ParsedUPITransaction | null {
  ...
  const confidence: "high" | "medium" | "low" =
    merchant !== "UPI Payment" && amount > 0 ? "high" :
    amount > 0 ? "medium" : "low";
...
export function markDuplicates(
  ...
  return parsed.map(p => {
    const isDuplicate = existing.some(e => {
      ...
      const similarMerchant = e.merchant?.toLowerCase().includes(
        p.merchant.toLowerCase().split(" ")[0]
      );
...
const BANK_COLUMN_PROFILES: { name: string; headers: string[]; map: ColumnMap }[] = [
  ...
  {
    name: "ICICI Bank",
    headers: ["s no", "transaction date", "cheque number", "particulars", "amount (dr)", "amount (cr)"],
    map: { date: "Transaction Date", narration: "Particulars", debit: "Amount (Dr)", credit: "Amount (Cr)" },
  },
  {
    name: "Axis Bank",
...
export function parseCSVStatement(csvText: string, bankHint?: string): ParsedUPITransaction[] {
  ...
  const profile = bankHint
    ? BANK_COLUMN_PROFILES.find(p => p.name.toLowerCase().includes(bankHint.toLowerCase())) ?? detectBankProfile(headers)
  ...
  for (let i = headerIdx + 1; i < lines.length; i++) {
    ...
    const amount = debit > 0 ? debit : credit > 0 ? credit
      : parseFloat(amtStr.replace(/[^0-9.]/g, "")) || 0;
    ...
    let merchant = narr;
    if (upiId) {
      merchant = upiId.split("@")[0].replace(/[-._]/g, " ")
...
export function generateRealisticMocks(): ParsedUPITransaction[] {
  const merchants = [
    ...
    { name: "Salary / ACME Corp", category: "Income", amounts: [45000, 55000, 30000], type: "credit" },
