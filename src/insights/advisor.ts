...
export function buildBriefing(
  ...
  debits.forEach(t => { byMerchant[t.merchant] = (byMerchant[t.merchant] ?? 0) + t.amount; });
  ...
  return {
    totalIncome, totalSpent, net, savingsRate,
    topCategories, topMerchants, subscriptionTotal,
    transactionCount: transactions.length,
    avgDailySpend, largestExpense,
...
function localAdvisor(
  ...
  if (/budget|over.?spend|deficit|limit/.test(q)) {
    if (isInDef) return `You're in a **deficit of ${C(Math.abs(b.net))}** this period. Your top two leaks are **${top?.name ?? "spending"}** (${C(top?.amount ?? 0)}) and **${top2?.name ?? "other"}** (${C(top2?.amount ?? 0)}). Start with a strict limit on just these two categories.\n\n[ACTION:CREATE_BUDGET]`;
  ...
  if (/sav(e|ing|ings)|investm|mutual fund|sip|fd|fixed deposit/.test(q)) {
    if (noIncome) return `No income logged yet. Add your salary or income source so I can calculate your savings rate.\n\n[ACTION:ADD_TRANSACTION]`;
    if (b.savingsRate < 0)  return `You're spending **${Math.abs(b.savingsRate)}% more than you earn**. Before saving, close the gap — cut ${C(Math.abs(b.net))} from monthly expenses. Your highest cost is **${top?.name}** at ${C(top?.amount ?? 0)}.\n\n[ACTION:CREATE_BUDGET]`;
    if (b.savingsRate < 10) return `Your savings rate is **${b.savingsRate}%** — below the healthy 20% benchmark. Redirect just ${C(Math.round((b.totalIncome * 0.2) - b.net))} more per month from **${top?.name}** to hit 20%. Consider a recurring SIP on your payday.\n\n[ACTION:SET_GOAL]`;
    if (b.savingsRate < 20) return `Savings rate: **${b.savingsRate}%** — getting there. To hit 20%, reduce **${top?.name}** spending by about ${C(Math.round(b.topCategories[0]?.amount * 0.15 || 0))} next month.\n\n[ACTION:SET_GOAL]`;
  ...
  if (/spend|spent|where|breakdown|categor|money go/.test(q)) {
    if (!top) return "No debit transactions yet. Start logging your expenses and I'll give you a full breakdown.";
    const catList = b.topCategories.slice(0, 3)
  ...
  if (/subscri|netflix|spotify|streaming|recurring/.test(q)) {
    if (b.subscriptionTotal === 0) return `No subscription transactions detected yet. Tag your recurring services as **Subscriptions** and I can give you an audit.\n\n[ACTION:VIEW_SUBSCRIPTIONS]`;
  ...
  if (/health|score|performance|rating|how am i doing/.test(q)) {
    const score =
      b.savingsRate >= 20 ? "Excellent 🟢" :
      b.savingsRate >= 10 ? "Good 🟡" :
      b.savingsRate >= 0  ? "Needs Work 🟠" : "Critical 🔴";
  ...
  if (/income|earn|salary|revenue|paych/.test(q)) {
    if (noIncome) return `No income transactions logged. Add your salary or any other income source with the + button.\n\n[ACTION:ADD_TRANSACTION]`;
  ...
  if (/goal|target|dream|plan|milestone/.test(q)) {
    if (b.net <= 0) return `To set meaningful goals, first close your current deficit of **${C(Math.abs(b.net))}**. Once you have a surplus, goals become achievable.\n\n[ACTION:CREATE_BUDGET]`;
  ...
  if (/unusual|anomal|weird|strange|different/.test(q)) {
    if (b.unusualCount === 0) return `No unusual spending patterns detected in your recent transactions. Everything looks consistent with your normal habits.\n\n[ACTION:VIEW_ANALYTICS]`;
  ...
  if (/merchant|shop|store|vendor|restaurant|amazon|flipkart|swiggy|zomato/.test(q)) {
    const top3m = b.topMerchants.slice(0, 3).map(m => `— **${m.name}**: ${C(m.amount)}`).join("\n");
  ...
  if (/advice|tip|help|suggest|recommend|how/.test(q)) {
    if (isInDef) return `My top suggestion: you're **${C(Math.abs(b.net))}** in deficit. Cut **${top?.name ?? "your top category"}** spending by 20% this week — that's about ${C(Math.round((top?.amount ?? 0) * 0.2))}. Small, consistent cuts beat big dramatic ones.\n\n[ACTION:CREATE_BUDGET]`;
  ...
  const isFinance = /money|spend|budget|sav|income|expense|transaction|invest|buy|afford|rich|poor|debt|loan|tax|goal|category|merchant|sub|bill/.test(q);
...
export function getProactiveNudge(
  ...
  const overBudgetCat = Object.entries(budgets).find(([, v]) => v.limit > 0 && v.spent / v.limit > 0.9);
  if (overBudgetCat) {
...
export function getSpendingPersonality(
  ...
  if (b.transactionCount < 5) {
    return {
      ...
      emoji:       "🌱",
      description: "You're just getting started! Log more transactions to unlock your spending personality.",
  ...
  if (topCat === "Food" && b.topCategories[0]?.percent > 35) return {
  ...
  if (topCat === "Shopping" && b.topCategories[0]?.percent > 30) return {
  ...
  if (savingsR < 5 && b.totalIncome > 0) return {
  ...
  if (topCat === "Transport" && b.topCategories[0]?.percent > 25) return {
...
export function generateQuests(
  ...
  if (uncategorized.length > 0) {
    quests.push({ id: "quest_uncat", title: "Cleanup Crew", description: `Categorize ${uncategorized.length} uncategorized transaction${uncategorized.length !== 1 ? "s" : ""}.`, reward: "+30 XP", type: "uncategorized", completed: false });
  }

  if (topCat) {
    quests.push({ id: "quest_topcat", title: `${topCat[0]} Diet`, description: `Avoid spending on ${topCat[0]} for the rest of today.`, reward: "+50 XP", type: "category", completed: false });
  }

  const loggedToday = transactions.filter(t => new Date(t.date) >= new Date(Date.now() - 86400000));
  if (loggedToday.length === 0) {
    quests.push({ id: "quest_log", title: "Active Tracker", description: "Log your first transaction today to keep your streak alive.", reward: "+20 XP", type: "logging", completed: false });
  } else {
    quests.push({ id: "quest_streak", title: "Streak Saver", description: `You've logged ${loggedToday.length} transaction${loggedToday.length !== 1 ? "s" : ""} today. Keep it up!`, reward: "+40 XP", type: "streak", completed: false });
  }

  if (credits.length > 0) {
    quests.push({ id: "quest_savings", title: "Rainy Day Fund", description: "Transfer 10% of your recent income to a savings goal.", reward: "+60 XP", type: "savings", completed: false });
  }

  const pool: GeneratedQuest[] = [
    { id: "quest_no_spend", title: "No-Spend Hour", description: "Go 3 hours without a discretionary purchase.", reward: "+25 XP", type: "streak", completed: false },
    { id: "quest_budget_checkin", title: "Budget Check-In", description: `Review your spending — you've spent ${C(Math.round(totalSpent))} total.`, reward: "+20 XP", type: "budget", completed: false },
    { id: "quest_savings_rate", title: "Savings Pulse", description: totalIncome > 0 ? `Your savings rate is ${Math.round(((totalIncome - totalSpent) / totalIncome) * 100)}%. Target 20%+.` : "Log an income transaction to calculate your savings rate.", reward: "+30 XP", type: "savings", completed: false },
    { id: "quest_income_log", title: "Paycheck Planner", description: "Record all income sources you received this week.", reward: "+45 XP", type: "logging", completed: false },
    { id: "quest_weekly_review", title: "Weekly Snapshot", description: "Check your Analytics view for your top spending categories.", reward: "+15 XP", type: "budget", completed: false },
    { id: "quest_goal_progress", title: "Goal Booster", description: "Add any amount to one of your active savings goals.", reward: "+50 XP", type: "savings", completed: false },
  ];
