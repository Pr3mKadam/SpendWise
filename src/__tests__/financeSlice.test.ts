...
function createTestStore(parentalOverrides: Partial<MockParentalState> = {}) {
  ...
  const store = createStore<Record<string, unknown>>()((set, get, api) => ({
    ...(createFinanceSlice as unknown as (s: typeof set, g: typeof get, a: typeof api) => FinanceSlice)(set, get, api),
    parentalState: parental,
    requestTransactionApproval: (t: Transaction) => { approvalSpy.push(t); },
...
describe('financeSlice', () => {
  describe('CRUD operations', () => {
    ...
    it('does nothing when updating category of non-existent transaction', () => {
      expect(() => store.getState().updateTransactionCategory('non-existent', 'Food')).not.toThrow();
    });
    ...
    it('does nothing when updating non-existent transaction', () => {
      expect(() => store.getState().updateTransaction('non-existent', { amount: 999 })).not.toThrow();
    });
  ...
  describe('budget limits calculations', () => {
    ...
    it('calculates total spend per category from transactions', () => {
      ...
      const transportTotal = store.getState().transactions
    ...
    it('alerts when spend exceeds budget', () => {
      ...
      const foodTotal = store.getState().transactions
    ...
    it('updates budget settings', () => {
      expect(store.getState().budgetSettings).toEqual({ period: 'monthly', rolloverEnabled: false });
  ...
  describe('transaction filtering and indexedData', () => {
    ...
    it('removes from byCategory index on delete', () => {
      store.getState().addTransaction(tx({ id: 'del-cat', category: 'Food', date: '2026-06-15T10:00:00Z' }));
      expect(store.getState().indexedData.byCategory['Food']).toHaveLength(1);
    ...
    it('updates byCategory index on category change', () => {
      store.getState().addTransaction(tx({ id: 're-cat', category: 'Food', date: '2026-06-15T10:00:00Z' }));
      expect(store.getState().indexedData.byCategory['Food']).toHaveLength(1);
  ...
  describe('recurring transactions', () => {
    ...
    it('adds a recurring transaction', () => {
      store.getState().addRecurringTransaction({
        id: 'rt-1', merchant: 'Rent', amount: 15000,
        category: 'Utilities', frequency: 'monthly',
        lastProcessed: null, nextOccurrence: '2026-03-01',
      });
  ...
  describe('subscriptions', () => {
    ...
    it('adds a subscription', () => {
      store.getState().addSubscription({
        merchant: 'Netflix', category: 'Subscriptions', avgAmount: 799,
        frequency: 'monthly', lastSeen: '2026-01-01',
        nextExpected: '2026-02-01', occurrences: 12, totalSpent: 9588,
      });
  ...
  describe('razorpay keys', () => {
    ...
    it('sets and clears razorpay keys', () => {
      ...
      expect(store.getState().razorpayKeys).toEqual({ keyId: 'rzp_test_123', keySecret: 'sk_test_456' });
  ...
  describe('scheduled transactions', () => {
    ...
    it('getScheduledTransactions returns only scheduled non-deleted', () => {
      const scheduled = tx({ id: 'sch-1', status: 'scheduled' as const, date: '2026-07-01T10:00:00Z' });
    ...
    it('processScheduledTransactions converts past-due scheduled to posted', () => {
      ...
      const future = tx({ id: 'future', status: 'scheduled' as const, date: '2099-12-31T10:00:00Z' });
