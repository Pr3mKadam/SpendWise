  ...
  describe('budget period calculations', () => {
    ...
    it('computes budgetStats from categoryLimits and budgets', async () => {
      setupMocks({
        ...
        transactions: [
          { id: '1', date: formatLocalYYYYMMDD(new Date()), amount: 500, category: 'Food' as const, merchant: 'Test', type: 'debit' as const, description: '' },
          { id: '2', date: formatLocalYYYYMMDD(new Date()), amount: 300, category: 'Food' as const, merchant: 'Test', type: 'debit' as const, description: '' },
          { id: '3', date: formatLocalYYYYMMDD(new Date()), amount: 200, category: 'Transport' as const, merchant: 'Test', type: 'debit' as const, description: '' },
        ],
      });
      const { useBudgets } = await import('@/hooks/useBudgets');
      const { result } = renderHook(() => useBudgets());
    ...
    it('marks budget as danger when 90%+ used', async () => {
      setupMocks({
        budgets: { Food: 1000 },
        transactions: [
          { id: '1', date: formatLocalYYYYMMDD(new Date()), amount: 950, category: 'Food' as const, merchant: 'Test', type: 'debit' as const, description: '' },
        ],
      });
      const { useBudgets } = await import('@/hooks/useBudgets');
      const { result } = renderHook(() => useBudgets());
    ...
    it('marks budget as warning when 75-89% used', async () => {
      setupMocks({
        budgets: { Food: 1000 },
        transactions: [
          { id: '1', date: formatLocalYYYYMMDD(new Date()), amount: 800, category: 'Food' as const, merchant: 'Test', type: 'debit' as const, description: '' },
        ],
      });
      const { useBudgets } = await import('@/hooks/useBudgets');
      const { result } = renderHook(() => useBudgets());
    ...
    it('computes overBudgetCount', async () => {
      setupMocks({
        ...
        transactions: [
          { id: '1', date: formatLocalYYYYMMDD(new Date()), amount: 950, category: 'Food' as const, merchant: 'Test', type: 'debit' as const, description: '' },
          { id: '2', date: formatLocalYYYYMMDD(new Date()), amount: 1900, category: 'Transport' as const, merchant: 'Test', type: 'debit' as const, description: '' },
        ],
      });
      const { useBudgets } = await import('@/hooks/useBudgets');
      const { result } = renderHook(() => useBudgets());
    ...
    it('supports rollover calculations', async () => {
      ...
      setupMocks({
        ...
        transactions: [
          { id: 'prev', date: formatLocalYYYYMMDD(prevMonthStart), amount: 600, category: 'Food' as const, merchant: 'Test', type: 'debit' as const, description: '' },
          { id: 'curr', date: formatLocalYYYYMMDD(today), amount: 200, category: 'Food' as const, merchant: 'Test', type: 'debit' as const, description: '' },
        ],
      });
      const { useBudgets } = await import('@/hooks/useBudgets');
      const { result } = renderHook(() => useBudgets());
