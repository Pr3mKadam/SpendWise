...
describe('detectRecurringPatterns', () => {
  ...
  it('detects weekly pattern', async () => {
    const fn = await testDetectRecurring();
    const result = fn([
      tx({ id: '1', date: '2024-06-03', amount: 500, merchant: 'Netflix', category: 'Entertainment' }),
      tx({ id: '2', date: '2024-06-10', amount: 500, merchant: 'Netflix', category: 'Entertainment' }),
      tx({ id: '3', date: '2024-06-17', amount: 500, merchant: 'Netflix', category: 'Entertainment' }),
    ]);
  ...
  it('detects monthly pattern', async () => {
    const fn = await testDetectRecurring();
    const result = fn([
      tx({ id: '1', date: '2024-04-15', amount: 1500, merchant: 'Rent Payment', category: 'Housing' }),
      tx({ id: '2', date: '2024-05-15', amount: 1500, merchant: 'Rent Payment', category: 'Housing' }),
    ]);
    expect(result.length).toBeGreaterThanOrEqual(1);
  ...
  it('ignores credit transactions', async () => {
    const fn = await testDetectRecurring();
    const result = fn([
      tx({ id: '1', date: '2024-06-01', amount: 5000, type: 'credit', merchant: 'Salary', category: 'Income' }),
    ]);
  ...
  it('requires minimum 2 occurrences', async () => {
    const fn = await testDetectRecurring();
    const result = fn([
      tx({ id: '1', date: '2024-06-10', amount: 250, merchant: 'Electric Bill', category: 'Bills & Utilities' }),
    ]);
    expect(result).toEqual([]);
  });
  ...
  it('merges similar merchant names (case-insensitive)', async () => {
    const fn = await testDetectRecurring();
    const result = fn([
      tx({ id: '1', date: '2024-06-03', amount: 100, merchant: 'Spotify Premium', category: 'Entertainment' }),
      tx({ id: '2', date: '2024-06-10', amount: 100, merchant: 'spotify_premium', category: 'Entertainment' }),
    ]);
...
describe('mergeSharedStorage', () => {
  interface MergeItem { id: string; name?: string; balance?: number; amount?: number; purpose?: string; created_by?: string }
  interface Storage { groups: MergeItem[]; members: MergeItem[]; walletEntries: MergeItem[]; expenses: MergeItem[]; goals: MergeItem[]; deleted_ids: string[] }

  const emptyStore = (): Storage => ({ groups: [], members: [], walletEntries: [], expenses: [], goals: [], deleted_ids: [] });
  ...
  function toShared(v: Storage): import('@/core/crdt').SharedStorage {
    const g = v.groups.map(g => ({ id: g.id, name: g.name ?? '', purpose: g.purpose ?? '', created_by: g.created_by ?? '' }));
    const m = v.members.map(m => ({ id: m.id, display_name: m.name ?? '', emoji: '', role: 'member', status: 'active', invited_at: '', group_id: '' }));
    const w = v.walletEntries.map(w => ({ id: w.id, kind: 'contribution' as const, amount: w.balance ?? 0, label: '', date: '', group_id: '', member_id: '' }));
    const e = v.expenses.map(e => ({ id: e.id, label: '', category: '', amount: e.amount ?? 0, date: '', paid_by_member_id: '', group_id: '' }));
    const gl = v.goals.map(gl => ({ id: gl.id, name: gl.name ?? '', emoji: '', target_amount: 0, target_date: '', color: '', group_id: '' }));
    return { groups: g, members: m, walletEntries: w, expenses: e, goals: gl, deleted_ids: v.deleted_ids };
  }

  it('merges unique items from both sides', async () => {
    const fn = await testMerge();
    const local = emptyStore(); local.groups = [item('g1', { name: 'Groceries' })];
    const remote = emptyStore(); remote.groups = [item('g2', { name: 'Transport' })];
  ...
  it('remote overwrites local on conflict (LWW)', async () => {
    const fn = await testMerge();
    const local = emptyStore(); local.groups = [item('g1', { name: 'Old Name' })];
    const remote = emptyStore(); remote.groups = [item('g1', { name: 'New Name' })];
    const result = fn(toShared(local), toShared(remote));
  ...
  it('removes deleted items via tombstones', async () => {
    const fn = await testMerge();
    const local = emptyStore(); local.groups = [item('g1', { name: 'Groceries' })]; local.deleted_ids = ['g1'];
  ...
  it('merges deleted_ids from both sides', async () => {
    const fn = await testMerge();
    const local = emptyStore(); local.deleted_ids = ['a', 'b'];
    const remote = emptyStore(); remote.deleted_ids = ['b', 'c'];
    const result = fn(toShared(local), toShared(remote));
  ...
  it('handles empty stores', async () => {
    ...
    expect(fn(toShared(empty), toShared(empty))).toEqual({
      groups: [], members: [], walletEntries: [], expenses: [], goals: [], deleted_ids: []
    });
  });
...
describe('error utilities', () => {
  ...
  it('ApiError contains code and message', async () => {
    ...
    const err = new ApiError({ code: 'NETWORK_ERROR', message: 'Network offline', retryable: true, status: 0 });
