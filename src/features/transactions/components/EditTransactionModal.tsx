...
function EditForm({ tx, onSave, onClose, currency }: {
  tx: Transaction;
  onSave: (id: string, updates: Partial<Omit<Transaction, 'id'>>) => void;
  onClose: () => void;
  currency: string;
  ...
  return (
    <>
      <div className="p-5 space-y-4">
        <div>
          <label className="text-[11px] font-semibold uppercase tracking-[0.06em] text-[var(--text-muted)] block mb-1.5">Merchant</label>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="text-[11px] font-semibold uppercase tracking-[0.06em] text-[var(--text-muted)] block mb-1.5">Amount ({currency})</label>
            <input type="number" step="0.01" min="0" value={amount} onChange={e => setAmount(e.target.value)}
              className="w-full p-3 rounded-xl bg-[var(--surface-input)] border border-[var(--border)] text-sm text-[var(--text-primary)] outline-none focus:border-[var(--teal)] transition-all" />
          </div>
          <div>
            <label className="text-[11px] font-semibold uppercase tracking-[0.06em] text-[var(--text-muted)] block mb-1.5">Type</label>
        ...
        <div>
          <label className="text-[11px] font-semibold uppercase tracking-[0.06em] text-[var(--text-muted)] block mb-1.5">Category</label>
        ...
        <div>
          <label className="text-[11px] font-semibold uppercase tracking-[0.06em] text-[var(--text-muted)] block mb-1.5">Date</label>
          <input type="date" value={date} onChange={e => setDate(e.target.value)}
            className="w-full p-3 rounded-xl bg-[var(--surface-input)] border border-[var(--border)] text-sm text-[var(--text-primary)] outline-none focus:border-[var(--teal)] transition-all" />
        </div>

        <div>
          <label className="text-[11px] font-semibold uppercase tracking-[0.06em] text-[var(--text-muted)] block mb-1.5">Receipt</label>
          {tx.receiptUrl ? (
            <div className="flex items-center gap-3 p-3 rounded-xl bg-[var(--surface-input)] border border-[var(--border)]">
              ...
              <span className="text-xs text-[var(--text-muted)] flex-1 truncate">Receipt attached</span>
              <button onClick={handleRemoveReceipt} disabled={uploading}
          ...
          <input ref={fileInputRef} type="file" accept="image/*" className="hidden" onChange={handleAttachReceipt} />
        </div>

        <div>
          <label className="text-[11px] font-semibold uppercase tracking-[0.06em] text-[var(--text-muted)] block mb-1.5">Description (optional)</label>
...
export function EditTransactionModal({ tx, isOpen, onClose, onSave, currency }: EditTransactionModalProps) {
  if (!isOpen || !tx) return null;

  return (
    <Portal>
      <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
        <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={onClose} onKeyDown={e => e.key === 'Escape' && onClose()} aria-hidden="true" />
        <div
          className="relative w-full max-w-md rounded-2xl shadow-2xl overflow-hidden"
          style={{ background: 'var(--surface-card)', border: '1px solid var(--border)' }} /* tailwind-migration:skip */
        >
          <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--border)]">
            <h3 className="font-manrope font-bold text-lg text-[var(--text-primary)]">Edit Transaction</h3>
            <button onClick={onClose} className="p-1 rounded-md hover:bg-[var(--surface-input)] text-[var(--text-muted)] cursor-pointer bg-transparent border-none" aria-label="Close">
              <X size={18} />
            </button>
          </div>
