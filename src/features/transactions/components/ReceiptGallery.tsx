    ...
    return (
      <div className="space-y-6">
        ...
        <EmptyState
          ...
          subMessage="Attach receipts when editing transactions to see them here."
        />
      </div>
    );
  }
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-caption mt-1">
          {withReceipts.length} receipt{withReceipts.length !== 1 ? 's' : ''} attached
        </p>
      </div>

      {grouped.map(group => (
        <div key={group.month}>
          <h3 className="text-[11px] font-black uppercase tracking-[0.12em] text-[var(--text-muted)] mb-3">
            {group.label}
          </h3>
