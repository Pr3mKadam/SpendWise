      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3 max-h-[300px] overflow-y-auto pr-1">
          {splits.map((sp, i) => (
            <div
              key={i}
              className="flex items-start gap-2 bg-[var(--surface-card)] border border-[var(--border)] rounded-xl p-3"
            >
              <div className="flex-1 space-y-2 min-w-0">
                <div className="flex gap-2">
                  <select
                    ...
                    style={{
                      color: CATEGORY_COLORS[sp.category] || 'var(--text-primary)',
        ...
        <div
          className={`flex items-center justify-between p-2.5 rounded-xl text-xs font-bold ${isBalanced ? 'bg-emerald-500/10 text-emerald-500' : 'bg-red-500/10 text-red-500'}`}
        >
          <span>{isBalanced ? '✓ Balanced' : `${currency}${remaining.toFixed(2)} remaining`}</span>
          <span>
            {currency}
            {allocated.toFixed(2)} / {currency}
            {totalAmount.toFixed(2)}
          </span>
        </div>
