...
export interface FinanceSlice {
  ...
  addMandate: (mandate: UPIMandate) => void;
  updateMandate: (id: string, data: Partial<UPIMandate>) => void;
  removeMandate: (id: string) => void;
  autoCategorizeMandate: (
    merchant: string,
    amount: number
  ) => { category: DefaultCategory; type: UPIMandate['type'] };
  undo: () => boolean;
  canUndo: () => boolean;
...
> = (set, get) => ({
  ...
  addMandate: mandate =>
    set(state => ({
      mandates: [...state.mandates, mandate],
    })),

  updateMandate: (id, data) =>
    set(state => ({
      mandates: state.mandates.map(m => (m.id === id ? { ...m, ...data } : m)),
    })),
  ...
  autoCategorizeMandate: (merchant, _amount) => {
    const m = merchant.toLowerCase();
    if (/loan|emi|finance|hdfc.*emi|icici.*emi|bajaj|tata.*capital|kredit/i.test(m))
      return { category: 'Debt' as DefaultCategory, type: 'emi' as const };
    if (/sip|mutual\s*fund|mf|hdfc.*mf|icici.*mf|invest/i.test(m))
      return { category: 'Investment' as DefaultCategory, type: 'sip' as const };
    if (/insurance|lic|icici.*prudential|hdfc.*life|max.*life/i.test(m))
      return { category: 'Insurance' as DefaultCategory, type: 'insurance' as const };
    if (/netflix|spotify|prime|youtube|hotstar|disney|zee|sony|linkedin/i.test(m))
      return { category: 'Subscriptions' as DefaultCategory, type: 'subscription' as const };
    return { category: 'Subscriptions' as DefaultCategory, type: 'other' as const };
  },
  ...
  bulkDeleteTransactions: ids => {
    ...
    set(state => ({
      transactions: state.transactions.map(t => (idSet.has(t.id) ? { ...t, deletedAt: now } : t)),
    }));
    get().reindex();
  },
