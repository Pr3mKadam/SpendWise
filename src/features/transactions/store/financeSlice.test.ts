...
describe('financeSlice', () => {
  describe('basic CRUD', () => {
    ...
    it('bulk deletes transactions (soft delete)', () => {
      ...
      expect(store.getState().transactions.find(t => t.id === 'keep-me')?.deletedAt).toBeUndefined();
    });
  ...
  describe('scheduled transactions', () => {
    it('getScheduledTransactions returns future-dated txns', () => {
      const store = createTestStore();
      store.getState().addTransaction(tx({ id: 'future', status: 'scheduled', date: '2099-01-01' }));
