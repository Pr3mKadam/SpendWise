...
export function PricingCard({ currentPlan = 'individual', onUpgrade, compact }: PricingCardProps) {
    ...
    return (
      <div className="card p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="font-manrope font-bold text-lg text-[var(--text-primary)]">Your Plan</h3>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-[900px] mx-auto">
      {PLANS.map(plan => {
        const planKey =
          plan.name === 'Pro' ? 'pro' : plan.name === 'Family' ? 'family' : 'individual';
            ...
            <ul className="space-y-3 mb-8 flex-1">
              {plan.features.map(f => (
                <li key={f.text} className="flex items-start gap-3">
                  ...
                  >
                    <Check size={12} style={{ color: f.included ? 'var(--teal)' : '#94a3b8' }} />
                  </div>
                  <span
                    style={{
                      fontFamily: 'var(--font-inter)',
