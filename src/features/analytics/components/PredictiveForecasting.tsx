...
function ForecastTooltip({ active, payload, label, currency }: any) {
  ...
  return (
    <div className="card px-3 py-2 shadow-lg border border-[var(--border)]">
      <p className="text-[length:var(--fs-overline)] font-bold text-[var(--text-muted)] mb-1">
        {label}
      </p>
      <p
        className="font-manrope font-bold text-sm"
        style={{ color: v >= 0 ? 'var(--teal)' : 'var(--red)' }}
      >
        {currency}
...
export function PredictiveForecasting({
  ...
  const data = useMemo(() => {
    ...
    const thirtyDaysAgo = formatLocalYYYYMMDD(new Date(now.getTime() - 30 * 86400000));
      ...
      <div className="flex items-start justify-between mb-5">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-[length:var(--fs-caption)] text-[var(--text-muted)] font-inter mt-0.5">
              AI-powered balance prediction · {data.daysLeft}d remaining
            </p>
          </div>
        </div>
        <div
          className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[length:var(--fs-overline)] font-bold ${isPositive ? 'bg-emerald-500/10 text-emerald-500' : 'bg-red-500/10 text-red-500'}`}
        >
        ...
        {[
          {
            ...
            value: `${currency}${Math.round(data.projectedEOM).toLocaleString('en-IN')}`,
            color: isPositive ? '#10b981' : '#ef4444',
          },
          {
            label: 'Daily Spend Rate',
            value: `${currency}${Math.round(data.dailySpendRate).toLocaleString('en-IN')}/day`,
            color: '#f59e0b',
          },
          {
            label: 'Net Daily Flow',
            value: `${data.dailyNetRate >= 0 ? '+' : ''}${currency}${Math.round(data.dailyNetRate).toLocaleString('en-IN')}`,
            color: data.dailyNetRate >= 0 ? '#10b981' : '#ef4444',
          },
        ].map(kpi => (
          <div
            key={kpi.label}
            className="bg-[var(--surface-input)] rounded-xl p-3 border border-[var(--border)]"
          >
            <p className="text-[length:var(--fs-overline)] font-bold uppercase tracking-wider text-[var(--text-muted)]">
              {kpi.label}
            </p>
            <p
              className="font-manrope font-bold text-sm mt-1 tabular-nums"
      ...
      {!isPositive && (
        ...
        >
          ...
          <p className="text-xs text-amber-200/90 font-inter leading-relaxed">
            At your current spending rate, you may end the month with a deficit. Consider reducing
            discretionary spending by {currency}
            {Math.round(Math.abs(data.dailyNetRate) * 5).toLocaleString('en-IN')}.
          </p>
        </motion.div>
      )}
    </div>
