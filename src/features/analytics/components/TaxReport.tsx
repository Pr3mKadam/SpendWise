...
export default function TaxReport({ transactions = [], currency = '\u20B9' }: TaxReportProps) {
  ...
  const totalDeductions = useMemo(
    () => Math.min(section80C, SECTION_80C_LIMIT) + Math.min(section80D, SECTION_80D_LIMIT) + hraExemption + Math.min(nps80CCD, NPS_80CCD_1B_LIMIT) + Math.min(homeLoan24b, HOME_LOAN_24B_LIMIT),
  ...
  const newTaxableIncome = useMemo(
...
<body>
  ...
  }, [income, oldTaxableIncome, newTaxableIncome, oldTax, newTax, oldTotal, newTotal, totalDeductions, section80C, section80D, hraExemption, nps80CCD, homeLoan24b, recommended, savings, currency]);
  ...
  }, [income, oldTotal, newTotal, recommended, savings, section80C, section80D, hraExemption, nps80CCD, homeLoan24b, currency]);
  ...
  return (
    <div className="view-enter max-w-4xl mx-auto space-y-6">
      ...
      >
        ...
        {showInputs ? <ChevronUp size={20} className="text-[var(--text-muted)]" /> : <ChevronDown size={20} className="text-[var(--text-muted)]" />}
      </button>
          ...
          <div className="rounded-2xl bg-[var(--surface-card)] border border-[var(--border)] p-6">
            ...
            {autoIncome > 0 && (
              <p className="text-[length:var(--fs-overline)] text-[var(--text-muted)] mt-2">
                Auto-filled from {new Date().getFullYear()} credit transactions: {currency}{autoIncome.toLocaleString('en-IN')}
              </p>
            )}
          </div>
          ...
          <div className="rounded-2xl bg-[var(--surface-card)] border border-[var(--border)] p-6 space-y-5">
              ...
              <div key={field.key}>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className="text-[length:var(--fs-overline)] font-bold text-[var(--text-muted)]">
                    Max: {currency}{field.limit.toLocaleString('en-IN')}
                  </span>
                </div>
                <div className="flex items-center gap-3">
                ...
                {field.value > field.limit && (
                  <p className="text-xs text-amber-500 mt-1">
                    Capped at {currency}{field.limit.toLocaleString('en-IN')} per ITR rules
                  </p>
                )}
              </div>
            ))}
            ...
            <div className="pt-2">
              <div className="flex items-center gap-2 mb-1.5">
                <span className="text-[var(--teal)]"><Home size={18} /></span>
                <label className="text-sm font-semibold text-[var(--text-primary)]">HRA Details</label>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <label className="flex items-center gap-3 p-2.5 rounded-xl bg-[var(--surface-input)] border border-[var(--border)] cursor-pointer">
                  ...
                  />
                  <span className="text-sm font-semibold text-[var(--text-primary)]">Metro City</span>
                </label>
              </div>
              {hraExemption > 0 && (
                <p className="text-xs text-[var(--teal)] mt-1">
                  HRA Exemption: {currency}{hraExemption.toLocaleString('en-IN')}
                </p>
              )}
            </div>
          </div>
      ...
      <div className="rounded-2xl bg-[var(--surface-card)] border border-[var(--border)] p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-[var(--text-primary)]">
              {currency}{oldTaxableIncome.toLocaleString('en-IN')}
            </span>
          </div>
        </div>
        ...
        <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div className="p-3 rounded-xl bg-[var(--surface-input)] border border-[var(--border)] text-center">
            <p className="text-[length:var(--fs-overline)] font-bold text-[var(--text-muted)] uppercase tracking-widest">Old Regime</p>
            <p className="text-lg font-bold text-[var(--text-primary)]">{currency}{oldTotal.toLocaleString('en-IN')}</p>
          </div>
          <div className="p-3 rounded-xl bg-[var(--surface-input)] border border-[var(--border)] text-center">
            <p className="text-[length:var(--fs-overline)] font-bold text-[var(--text-muted)] uppercase tracking-widest">New Regime</p>
            <p className="text-lg font-bold text-[var(--text-primary)]">{currency}{newTotal.toLocaleString('en-IN')}</p>
          </div>
          <div className="p-3 rounded-xl bg-[var(--surface-input)] border border-[var(--border)] text-center">
            <p className="text-[length:var(--fs-overline)] font-bold text-[var(--text-muted)] uppercase tracking-widest">Deductions</p>
            <p className="text-lg font-bold text-[var(--text-primary)]">{currency}{totalDeductions.toLocaleString('en-IN')}</p>
          </div>
          <div className={cn(
            'p-3 rounded-xl border text-center',
            recommended === 'Old' ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-blue-500/10 border-blue-500/20'
          )}>
            <p className="text-[length:var(--fs-overline)] font-bold uppercase tracking-widest text-[var(--text-muted)]">Best Regime</p>
            <p className={cn(
              'text-lg font-bold',
              recommended === 'Old' ? 'text-emerald-600' : 'text-blue-500'
            )}>
              {recommended} ({currency}{savings.toLocaleString('en-IN')})
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-2xl bg-[var(--surface-card)] border border-[var(--border)] overflow-hidden">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <span className="font-bold text-[var(--text-primary)]">Suggested Tax-Saving Actions</span>
          </div>
          {showSuggestions ? <ChevronUp size={20} className="text-[var(--text-muted)]" /> : <ChevronDown size={20} className="text-[var(--text-muted)]" />}
        </button>
        {showSuggestions && (
          <motion.div
            initial={{ opacity: 0 }}
