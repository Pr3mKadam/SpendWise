  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div
          className={`p-4 rounded-2xl border ${regimeSavings > 0 ? 'bg-green-500/10 border-green-500/20' : 'bg-gray-500/10 border-gray-500/20'}`}
        >
          <p
            className={`text-[length:var(--fs-overline)] font-bold uppercase tracking-widest mb-1 ${regimeSavings > 0 ? 'text-green-600' : 'text-gray-500'}`}
          >
            {regimeSavings > 0 ? 'Regime Savings' : 'Optimal Choice'}
          </p>
          <p
            className={`text-xl font-bold ${regimeSavings > 0 ? 'text-green-600' : 'text-[var(--text-primary)]'}`}
          >
            {regimeSavings > 0
              ? `+${currency}${Math.round(regimeSavings).toLocaleString()}`
              : 'Best Choice'}
          </p>
          <p
            className={`text-[length:var(--fs-overline)] mt-1 ${regimeSavings > 0 ? 'text-green-700/70' : 'text-gray-500'}`}
          >
