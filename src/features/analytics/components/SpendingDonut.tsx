...
export default function SpendingDonut({ data, totalSpent, currency = '$' }: SpendingDonutProps) {
  ...
  if (chartData.length === 0) {
    return (
      <div
        className="card flex flex-col items-center justify-center h-[360px] text-center"
        role="region"
        aria-label="Expenses Breakdown"
      >
        <div className="text-3xl mb-3 opacity-40">🍩</div>
        <p className="text-title" style={{ color: 'var(--text-muted)' }}>
          No expenses yet
        </p>
  ...
  return (
    <div
      className="card px-5 py-5 flex flex-col"
      role="region"
      aria-label="Expenses Breakdown Chart Card"
    >
      ...
      <div
        className="relative h-[240px] shrink-0 my-4"
        role="img"
        aria-label={`Spending breakdown pie chart. Total expenses: ${currency}${totalSpent.toLocaleString()}.`}
      >
