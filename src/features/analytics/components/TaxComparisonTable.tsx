import { TaxSlab, calculateTax, calculateCess, calculateSurcharge, formatIndianCurrency } from '@/utils/taxConstants';
...
export function TaxComparisonTable({
  ...
  const allSlabs = [
    ...oldSlabs.map((s, i) => ({
      label: s.label,
      oldRate: `${s.rate}%`,
      newRate: newSlabs[i] ? `${newSlabs[i].rate}%` : '\u2014',
    })),
    ...(oldSlabs.length > newSlabs.length
      ? newSlabs.length > 0
        ? []
        : []
      : newSlabs.slice(oldSlabs.length).map(s => ({
          label: s.label,
          oldRate: '\u2014',
          newRate: `${s.rate}%`,
        }))),
    ...
    <div className="space-y-4">
      <div className="overflow-x-auto">
        <table className="w-full text-sm border-collapse">
          ...
          <tbody>
            {oldSlabs.map((slab, i) => {
              const oldSlabTax = calculateTax([slab], Math.min(oldTaxableIncome, slab.max));
              const newSlab =
                i < newSlabs.length
                  ? newSlabs[i]
                  : { min: slab.min, max: slab.max, rate: 0, label: slab.label };
              const newSlabTax = calculateTax([newSlab], Math.min(newTaxableIncome, newSlab.max));
              return (
                <tr key={i} className="border-b border-[var(--border)]/50 hover:bg-[var(--surface-input)]/30 transition-colors">
            ...
            {newSlabs.length > oldSlabs.length &&
              newSlabs.slice(oldSlabs.length).map((slab, i) => (
                <tr key={`extra-${i}`} className="border-b border-[var(--border)]/50 hover:bg-[var(--surface-input)]/30 transition-colors">
                  <td className="py-2.5 px-4 font-medium text-[var(--text-primary)]">
                    {slab.label}
                  </td>
          ...
          <div className="space-y-1 text-sm">
            <div className="flex justify-between">
              ...
              <span className="font-semibold text-[var(--text-primary)]">
                {currency}{oldTax.toLocaleString('en-IN')}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-[var(--text-muted)]">Health & Education Cess (4%)</span>
              <span className="font-semibold text-[var(--text-primary)]">
                {currency}{oldCess.toLocaleString('en-IN')}
              </span>
            </div>
            {oldSurcharge > 0 && (
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-[var(--text-primary)]">
                  {currency}{oldSurcharge.toLocaleString('en-IN')}
                </span>
              </div>
            )}
            <div className="flex justify-between pt-2 border-t border-[var(--border)]">
              ...
              <span className="font-bold text-[var(--text-primary)]">
                {currency}{oldTotal.toLocaleString('en-IN')}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl bg-[var(--surface-input)] border border-[var(--border)] p-4 space-y-2">
          ...
          <div className="space-y-1 text-sm">
            <div className="flex justify-between">
              <span className="text-[var(--text-muted)]">Base Tax</span>
              <span className="font-semibold text-[var(--text-primary)]">
                {currency}{newTax.toLocaleString('en-IN')}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-[var(--text-muted)]">Health & Education Cess (4%)</span>
              <span className="font-semibold text-[var(--text-primary)]">
                {currency}{newCess.toLocaleString('en-IN')}
              </span>
            </div>
            {newSurcharge > 0 && (
              <div className="flex justify-between">
                <span className="text-[var(--text-muted)]">Surcharge</span>
                <span className="font-semibold text-[var(--text-primary)]">
                  {currency}{newSurcharge.toLocaleString('en-IN')}
                </span>
              </div>
            )}
            <div className="flex justify-between pt-2 border-t border-[var(--border)]">
              <span className="font-bold text-[var(--text-primary)]">Total Liability</span>
              <span className="font-bold text-[var(--text-primary)]">
                {currency}{newTotal.toLocaleString('en-IN')}
              </span>
            </div>
          </div>
        </div>
