...
export function calculateHealthScore(
  ...
  const emergencyScore =
    targetEmergencyFund > 0 ? Math.min(100, (currentBalance / targetEmergencyFund) * 100) : 100;

  // Final Weighted Score
  const finalScore = Math.round(
    savingsScore * 0.4 + stabilityScore * 0.3 + disciplineScore * 0.2 + emergencyScore * 0.1
  );
  ...
  if (savingsScore < 70)
    recommendations.push(
      'Increase your savings rate by setting up an automated transfer of 5% more this month.'
    );
  if (stabilityScore < 70)
    recommendations.push(
      'Your liquidity is low. Aim to build a 3-month cash runway to handle emergencies.'
    );
  if (disciplineScore < 80)
    recommendations.push(
      "Discretionary spending is high. Consider the '24-hour rule' before making non-essential purchases."
    );
  if (emergencyScore < 100)
    recommendations.push(
      `Build your emergency fund to $${(monthlyExpenses * 3).toLocaleString()} for full security.`
    );
