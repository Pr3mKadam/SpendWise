    ...
    >
      ...
      {completed && (
        <div
          ...
          style={{
            background: `radial-gradient(circle at top right, ${lesson.color}, transparent 70%)`,
          }}
        />
      )}
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            <span
              className="text-[length:var(--fs-overline)] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full flex items-center gap-1"
              style={{ background: lesson.color + '12', color: lesson.color }}
            >
              {cfg.icon} {cfg.label}
            </span>
          ...
          <h4 className="font-manrope font-bold text-[15px] text-[var(--text-primary)] leading-snug">
            {lesson.title}
          </h4>
          <p className="font-inter text-[12px] text-[var(--text-secondary)] mt-1 leading-relaxed line-clamp-2">
            {lesson.summary}
          </p>
          {lesson.level > 1 && (
            <p className="text-[length:var(--fs-overline)] font-bold text-[var(--text-dim)] mt-2">
