...
import { MEMBER_EMOJIS, GOAL_EMOJIS, GOAL_COLORS, GROUP_PURPOSES } from '@/constants';
...
export function ExpenseModal({ show, onClose, members, onSubmit, currency }: {
  ...
  useEffect(() => {
    if (active.length > 0) {
      ...
      active.forEach((m, i) => {
        initial[m.id] = i === active.length - 1 ? Math.round((100 - p * (active.length - 1)) * 100) / 100 : p;
      });
  ...
  async function submit(e: React.FormEvent) {
    ...
    setBusy(true); setErr('');
    try {
      await onSubmit({ paidByMemberId: paidBy, label: label.trim(), category: cat, amount: a, date, splits: splitsToSave });
      setLabel(''); setAmount(''); setSplitMode('equal'); onClose();
            ...
            <div className="flex flex-col gap-3 mt-4">
              {active.map(m => (
                <div key={m.id} className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full bg-[var(--card)] flex items-center justify-center text-[1rem] shrink-0 border border-[var(--card-border)]">{m.emoji}</div>
                  <p className="m-0 flex-1 min-w-0 text-[0.8rem] font-semibold text-[var(--text)] truncate">{m.display_name}</p>
                  <div className="flex items-center gap-2 shrink-0">
                    <div className="relative">
                      ...
                      />
                      <span className="absolute right-2 top-1/2 -translate-y-1/2 text-[0.7rem] text-[var(--text-secondary)] font-bold opacity-50">%</span>
                    </div>
                    {amount && !isNaN(parseFloat(amount)) && (
                      <span className="w-16 text-right text-[0.75rem] font-bold text-[var(--text-secondary)]">
                        {currency}{((parseFloat(amount) * (customSplits[m.id] || 0)) / 100).toFixed(2)}
                      </span>
                    )}
                  </div>
                </div>
              ))}
              <div className={`mt-2 p-2 rounded-lg text-center text-[0.75rem] font-bold transition-colors ${Math.abs(customTotal - 100) < 0.1 ? 'bg-emerald-500/10 text-emerald-500' : 'bg-red-500/10 text-red-500'}`}>
                Total: {customTotal.toFixed(1)}% {Math.abs(customTotal - 100) > 0.1 && '(Must equal 100%)'}
              </div>
            </div>
          )}
        </div>

        <Btn full v="primary" type="submit" disabled={busy || !label.trim() || !amount || active.length === 0 || (splitMode === 'custom' && Math.abs(customTotal - 100) > 0.1)}>
  ...
  return (
    <Modal show={show} onClose={onClose} title={`${goal.emoji} Contribute to ${goal.name}`}>
      ...
      <form onSubmit={submit}>
        ...
        <Field label={`Amount (${currency})`}><Inp type="number" min="0.01" step="0.01" placeholder="0.00" value={amount} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setAmt(e.target.value)} /></Field>
        <Field label="Date"><Inp type="date" value={date} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setDate(e.target.value)} /></Field>
