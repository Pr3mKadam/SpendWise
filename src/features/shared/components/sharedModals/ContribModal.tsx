...
export function ContribModal({
  ...
  onSubmit: (goalId: string, memberId: string, amount: number, date: string, note?: string) => Promise<void>;
  currency: string;
}) {
  ...
  if (!goal) return null;
  const saved = (goal.contributions ?? []).reduce((s: number, c: SharedContribution) => s + c.amount, 0);
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      setErr(ex instanceof Error ? ex.message : 'Failed to add contribution.');
    } finally {
      setBusy(false);
    }
  }
    ...
    <Modal show={show} onClose={onClose} title={`${goal.emoji} Contribute to ${goal.name}`}>
      <div className="p-4 bg-[var(--bg)] rounded-xl mb-5">
        <div className="flex justify-between text-[0.78rem] text-[var(--text-secondary)] mb-2">
          ...
          <span style={{ color: goal.color }} /* tailwind-migration:skip */ /* tailwind-migration:skip */>
        ...
        <div className="h-2 rounded-full bg-[var(--card-border)] overflow-hidden">
          <div
            ...
            style={{ width: `${pct}%`, background: goal.color }} /* tailwind-migration:skip */
          />
        </div>
      </div>
