...
export function ExpenseModal({
  ...
  onSubmit: (p: { paidByMemberId: string; label: string; category: string; amount: number; date: string; splits: { memberId: string; sharePercent: number }[] }) => Promise<void>;
  currency: string;
}) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    let splitsToSave: { memberId: string; sharePercent: number }[];
    if (splitMode === 'equal') {
    ...
    try {
      ...
      setErr(ex instanceof Error ? ex.message : 'Failed to add expense.');
    } finally {
      setBusy(false);
    }
  }
