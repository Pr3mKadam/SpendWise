...
export type {
  SharedGroup, SharedGroupMember, SharedWalletEntry,
  SharedExpense, SharedExpenseSplit, SharedGoal, SharedGoalContribution
};
...
export function useSharedWallets(
  ...
  const walletEntries = data.walletEntries.filter(w => w.group_id === selectedGroupId && !activeIds.has(w.id));
  const expenses     = data.expenses.filter(e => e.group_id === selectedGroupId && !activeIds.has(e.id));
  ...
  const pendingInvites: PendingInvite[] = useMemo(() => {
    ...
    return data.members
      ...
      .map(inv => {
        ...
        return { memberId: inv.id, groupId: g.id, groupName: g.name, groupPurpose: g.purpose, invitedAt: inv.invited_at };
      })
  ...
  // ── Mutate helper ─────────────────────────────────────────────────
  const mutate = useCallback((updater: (prev: SharedStorage) => SharedStorage) => {
    setData(prev => {
  ...
  const addWalletEntry = useCallback(async (payload: {
    memberId: string; kind: SharedWalletEntry['kind'];
  ...
  const addExpense = useCallback(async (payload: {
    paidByMemberId: string; label: string; category: string;
    ...
    mutate(prev => ({
      ...
      goals: prev.goals.map(g => g.id !== goalId ? g : {
        ...
        contributions: [...(g.contributions || []), {
          id: uid(), goal_id: goalId, member_id: memberId, amount, date, note,
        }],
      }),
    }));
  }, [mutate]);
  ...
  const exportGroup = useCallback((groupId: string): string | null => {
  ...
  const importGroup = useCallback(async (encodedData: string): Promise<boolean> => {
    try {
      let decoded: any;
      try { decoded = JSON.parse(decodeURIComponent(atob(encodedData))); }
      catch { decoded = JSON.parse(atob(encodedData)); }
      ...
      mutate(prev => {
        ...
        return {
          ...
          walletEntries:[...prev.walletEntries.filter(w => w.group_id !== id), ...decoded.walletEntries],
  ...
  return {
    // Data
    groups, selectedGroupId, selectedGroup,
    members, walletEntries, expenses, goals, pendingInvites,
    walletBalance, splitBalances,
    loading: false, error,
    ...
    inviteMember, acceptInvite, declineInvite, removeMember,
    addWalletEntry, deleteWalletEntry,
