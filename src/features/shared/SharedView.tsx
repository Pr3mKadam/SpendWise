...
class SharedErrorBoundary extends Component<{ children: ReactNode }, { error: Error | null }> {
  ...
  componentDidCatch(error: Error, info: any) {
    console.error('[SharedView] Render error:', error, info);
  }
  render() {
    if (this.state.error) {
      return (
        <div className="flex flex-col items-center justify-center py-20 px-6 text-center bg-[var(--surface-card)] rounded-[28px] border border-[var(--border)] max-w-[500px] mx-auto shadow-sm">
          ...
          <h2 className="m-0 mb-2.5 text-lg font-extrabold text-[var(--text-primary)] tracking-tight">
            Something went wrong
          </h2>
          <p className="text-xs text-[var(--text-muted)] max-w-[360px] mb-6 leading-relaxed font-medium">
            The Shared Wallets view encountered an error. This is usually caused by a database
            connection issue.
          </p>
          ...
          <Btn v="primary" onClick={() => this.setState({ error: null })} className="mt-6 px-6">
            Try Again
          </Btn>
        </div>
      );
...
export default function SharedView({
  ...
  const isOwner = useMemo(
    () => sw.selectedGroup?.created_by === userId,
    [sw.selectedGroup, userId]
  );
  const groupName = sw.selectedGroup?.name || 'Shared Wallet';

  const openContrib = useCallback((g: SharedGoal) => {
    setActiveGoal(g);
    setContrib(true);
  }, []);
  ...
  return (
    <SharedErrorBoundary>
      <div className="animate-fade-in-up">
        <InviteBanner
          invites={sw.pendingInvites}
          onAccept={sw.acceptInvite}
          onDecline={sw.declineInvite}
        />
        ...
        {!sw.loading && sw.groups.length === 0 && sw.pendingInvites.length === 0 ? (
          ...
          <>
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 flex-wrap">
              <GroupSelector
                groups={sw.groups}
                selectedId={sw.selectedGroupId}
                onSelect={sw.setSelectedGroupId}
              ...
              <div className="flex items-center gap-3.5 bg-[var(--surface-card)] border border-[var(--border)] rounded-2xl px-4 py-2 text-xs shadow-sm flex-wrap">
                <div
                  ...
                  title="Group syncing happens automatically over encrypted P2P"
                >
                  <span className="relative flex h-2.5 w-2.5">
                    {sw.syncState === 'connecting' && (
                      <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-amber-400 opacity-75"></span>
                    )}
                    <span
                      className={`relative inline-flex rounded-full h-2.5 w-2.5 ${sw.syncState === 'connected' ? 'bg-emerald-500 shadow-sm shadow-emerald-500/50' : sw.syncState === 'connecting' ? 'bg-amber-500' : 'bg-red-500'}`}
                    ></span>
                  </span>
                  <span className="font-bold text-[var(--text-primary)]">Live Sync</span>
                  <span className="text-[var(--text-muted)] font-medium">
                    {sw.syncState === 'connected'
                      ? `${sw.connectedPeers} peers online`
                      : sw.syncState}
                  </span>
                </div>
              </div>
            ...
            {sw.loading && (
              <div className="text-center py-20 text-[var(--text-muted)] bg-[var(--surface-card)] rounded-[24px] border border-[var(--border)]">
                <div className="text-[2rem] inline-block mb-3 text-[var(--teal)] animate-spin">
                  <Ico.Spin />
                </div>
            ...
            {!sw.loading && sw.selectedGroupId && (
              <>
                ...
                >
                  ...
                  {tab === 'wallet' && (
                    <WalletTab
                      entries={sw.walletEntries}
                      members={sw.members}
                      onDelete={sw.deleteWalletEntry}
                      currency={currency}
                    />
                  )}
                  {tab === 'expenses' && (
                    <ExpensesTab
                      ...
                      members={sw.members}
                      splitBalances={sw.splitBalances}
                      onDelete={sw.deleteExpense}
                      currency={currency}
                    />
                  )}
                  {tab === 'goals' && (
        ...
        <CreateGroupModal
          ...
          onSubmit={async (n: string, p: string, e: string) => {
        ...
        <InviteModal
          ...
          onSubmit={sw.inviteMember}
          groupName={groupName}
          groupId={sw.selectedGroupId || undefined}
        />
        <GroupQRModal
          show={showQR}
          onClose={() => setQR(false)}
          groupData={sw.selectedGroupId ? sw.exportGroup(sw.selectedGroupId) || '' : ''}
          groupName={groupName}
        />
        <WalletModal
          show={showWallet}
        ...
        <ExpenseModal
          ...
          members={sw.members}
          onSubmit={sw.addExpense}
          currency={currency}
        />
        <GoalModal show={showGoal} onClose={() => setGoal(false)} onSubmit={sw.addGoal} />
        <ContribModal
          show={showContrib}
          onClose={() => setContrib(false)}
          goal={activeGoal}
          members={sw.members}
          onSubmit={sw.contributeToGoal}
          currency={currency}
        />
      </div>
    </SharedErrorBoundary>
