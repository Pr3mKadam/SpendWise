...
const STATUS_CONFIG: Record<
  BudgetStatus,
  { color: string; label: string; bg: string; text: string; icon: React.ElementType }
> = {
  safe: {
    color: '#14b8a6',
    label: 'On Track',
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 min-w-0">
              ...
              {hasRollover && (
                ...
                >
                  <RefreshCw size={9} />+{currency}
                  {budget.rolloverAmount.toFixed(0)}
                </span>
              )}
              {isSynced && (
                <span
          ...
          >
            <span style={{ color: 'var(--text-muted)' }}>
              ...
              of {currency}
              {budget.limit.toLocaleString('en-US', { maximumFractionDigits: 0 })}
              {hasRollover && (
                <span style={{ color: 'var(--text-muted)', opacity: 0.7 }}>
                  {' '}
                  ({currency}
                  {budget.baseLimit.toLocaleString('en-US', { maximumFractionDigits: 0 })} base)
                </span>
              )}
            </span>
