...
export function useBudgetManager({
  ...
  const availableCategories = allCategories.filter(
    (c: string) => !existingCategories.has(c as Category)
  );
  ...
  const unappliedSuggestions = suggestions.filter(
    (s: BudgetSuggestion) => !existingCategories.has(s.category as Category)
  );
  ...
  function handleApplyAll() {
    unappliedSuggestions.forEach((s: BudgetSuggestion) =>
      onUpdateLimit(s.category as Category, s.suggestedLimit)
    );
