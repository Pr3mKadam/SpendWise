...
export function useDashboardData(
  ...
  const recentMerchants = useMemo(() => {
    ...
    for (const tx of transactions) {
      if (!seen.has(tx.merchant)) {
        seen.add(tx.merchant);
        result.push(tx.merchant);
      }
  ...
  const subSpend = useMemo(() => {
    ...
    return transactions
      .filter(
        t => t.type === 'debit' && t.category === 'Subscriptions' && t.date.startsWith(monthStr)
      )
  ...
  const insights = useMemo(() => {
    ...
    const thisMonthTx = transactions.filter(
      t => t.date.startsWith(thisMonthStr) && t.type === 'debit'
    );
    const prevMonthTx = transactions.filter(
      t => t.date.startsWith(prevMonthStr) && t.type === 'debit'
    );
