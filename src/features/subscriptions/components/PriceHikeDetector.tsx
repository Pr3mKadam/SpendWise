    ...
    >
      <div className="card w-full max-w-md p-5 shadow-2xl" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            onClick={onClose}
            className="p-1 hover:text-red-500 text-[var(--text-muted)] bg-transparent border-none cursor-pointer"
          >
            <X size={18} />
          </button>
        </div>
        ...
        <div className="flex gap-3 mt-4">
          <button
            onClick={handleCopy}
            className="flex-1 py-2.5 rounded-xl font-bold text-sm border border-[var(--border)] text-[var(--text-primary)] bg-[var(--surface-input)] hover:border-[var(--teal)] hover:text-[var(--teal)] transition-all cursor-pointer"
          >
          ...
          <button
            onClick={() =>
              window.open(
                `mailto:support@${merchant.toLowerCase().replace(/\s+/g, '')}.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`
              )
            }
...
export function PriceHikeDetector({ transactions, currency }: PriceHikeDetectorProps) {
  ...
  const hikeAlerts = useMemo((): HikeAlert[] => {
    ...
    return Object.entries(merchantMap)
      .reduce<HikeAlert[]>((acc, [merchant, data]) => {
        if (data.amounts.length < 2) return acc;
        const oldAvg =
          data.amounts.slice(0, -1).reduce((a, b) => a + b, 0) / (data.amounts.length - 1);
        ...
        if (pct > 10)
          acc.push({
            merchant,
            oldAmount: Math.round(oldAvg * 100) / 100,
            newAmount: newAmt,
    ...
    return (
      <div className="flex items-center gap-3 p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/20">
        ...
        <div>
          ...
          <p className="text-[length:var(--fs-caption)] text-emerald-400/70 font-inter">
            Your recurring charges appear stable
          </p>
        </div>
      </div>
    );
    ...
    <>
      <div className="space-y-3">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="px-2 py-0.5 rounded-full bg-amber-500/10 text-amber-500 text-[length:var(--fs-overline)] font-bold">
            {hikeAlerts.length} detected
          </span>
        </div>
        <AnimatePresence>
          {visible.map(alert => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className="px-1.5 py-0.5 rounded-full bg-red-500/10 text-red-400 text-[length:var(--fs-overline)] font-bold">
                    +{alert.changePct}%
                  </span>
                </div>
                <p className="text-[length:var(--fs-caption)] text-[var(--text-muted)] font-inter">
                  Was {currency}
                  {alert.oldAmount} → Now {currency}
                  {alert.newAmount}
                </p>
              </div>
              <button
                onClick={() => setCancelTarget(alert.merchant)}
      ...
      <AnimatePresence>
        {cancelTarget && (
          <CancellationEmail merchant={cancelTarget} onClose={() => setCancelTarget(null)} />
        )}
      </AnimatePresence>
    </>
  );
