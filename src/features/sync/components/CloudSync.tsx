...
export function CloudSync({ transactions, onPullTransactions }: CloudSyncProps) {
  ...
  }, [user, transactions, lastSync, totalXP, level, streak]);
      ...
      <div className="card p-4 sm:p-5">
        ...
        <div className="bg-[var(--surface-input)] border border-[var(--border)] rounded-xl p-4 space-y-2">
          ...
          <div className="bg-[var(--surface-card)] rounded-lg p-3 mt-2 overflow-x-auto">
            ...
            <code className="text-[length:var(--fs-overline)] font-mono text-[var(--teal)] block break-all whitespace-pre-wrap mt-1">VITE_SUPABASE_ANON_KEY=eyJ...</code>
          </div>
          ...
          <p className="text-[length:var(--fs-overline)] text-[var(--text-dim)] font-inter mt-2 pt-2 border-t border-[var(--border)]">
            💡 <strong>Note:</strong> If you already configured these in your <code className="bg-[var(--surface-card)] px-1 rounded">.env</code> / <code className="bg-[var(--surface-card)] px-1 rounded">.env.local</code> file, you MUST <strong>restart your development server</strong> (or rebuild the app) for Vite to load them.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="card p-4 sm:p-5">
      ...
      <div className="flex items-start gap-3 mb-4">
        <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${user ? 'bg-emerald-500/10' : 'bg-slate-500/10'}`}>
          {user ? <Cloud size={18} className="text-emerald-500" /> : <CloudOff size={18} className="text-slate-400" />}
        </div>
        <div>
          <h3 className="font-manrope font-bold text-sm text-[var(--text-primary)]">Cloud Sync</h3>
        ...
        <span className={`ml-auto px-2 py-0.5 rounded-full text-[length:var(--fs-overline)] font-bold flex items-center gap-1 ${user ? 'bg-emerald-500/10 text-emerald-500' : 'bg-slate-500/10 text-slate-400'}`}>
          {user ? <><Wifi size={10} /> Active</> : <><WifiOff size={10} /> Off</>}
        </span>
      </div>
      ...
      <AnimatePresence mode="wait">
        ...
        {/* Not signed in — CTA */}
        {!user && mode === 'idle' && (
          <motion.div key="cta" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-2 mb-4">
        ...
        {user && (
          <motion.div key="sync-panel" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3">
            {/* Stats strip */}
