...
export function UPILink({ onSetView, onUPILinkSuccess }: UPILinkProps) {
  ...
  const [upiError, _setUpiError] = useState('');
    ...
    <div className="max-w-md mx-auto py-8 animate-scale-in">
      ...
      {wizardStep === 'upi-select' && (
        <div className="space-y-4">
          <h2 className="text-2xl font-manrope font-bold text-[var(--text-primary)] mb-6">
            Select App
          </h2>
          {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
          {UPI_PROVIDERS.map((p: any) => (
            <button
              ...
              className="w-full flex items-center gap-4 p-4 rounded-xl bg-[var(--surface-card)] border border-[var(--border)] hover:border-[var(--teal)] transition-all cursor-pointer"
            >
              <div
      ...
      {wizardStep === 'upi-credentials' && selectedProvider && (
        <div className="text-center">
          <div
            className="w-20 h-20 rounded-3xl mx-auto mb-6 flex items-center justify-center text-white text-3xl shadow-lg"
            style={{ background: selectedProvider.color }}
          >
            {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
            {(selectedProvider as any).icon || selectedProvider.name.charAt(0)}
          </div>
          <h3 className="text-xl font-manrope font-bold mb-2">Connect {selectedProvider.name}</h3>
          <p className="text-sm text-[var(--text-muted)] mb-8">
            Enter your UPI ID to sync your history.
          </p>
          <input
            type="text"
            value={upiId}
            ...
            className="w-full p-4 rounded-xl bg-[var(--surface-input)] border border-[var(--border)] focus:border-[var(--teal)] outline-none font-inter text-center text-lg mb-4"
          />
          {upiError && <p className="text-red-500 text-xs mb-4">{upiError}</p>}
          <button
            onClick={handleVerifyAndLink}
            className="w-full py-4 rounded-xl bg-[var(--teal)] text-white font-bold border-none cursor-pointer"
          >
            Verify & Link
          </button>
        </div>
