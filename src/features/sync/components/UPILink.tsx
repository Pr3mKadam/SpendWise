    ...
    <div className="max-w-md mx-auto py-8 animate-scale-in">
        ...
        <div className="space-y-4">
          ...
          {UPI_PROVIDERS.map((p: any) => (
            <button key={p.id} onClick={() => { setSelectedProvider(p); setWizardStep('upi-credentials'); }} className="w-full flex items-center gap-4 p-4 rounded-xl bg-[var(--surface-card)] border border-[var(--border)] hover:border-[var(--teal)] transition-all cursor-pointer">
              <div className="w-10 h-10 rounded-full flex items-center justify-center text-white shrink-0" style={{ background: p.color }}>{(p as any).icon || p.name.charAt(0)}</div>
      ...
      {wizardStep === 'upi-credentials' && selectedProvider && (
        <div className="text-center">
          <div className="w-20 h-20 rounded-3xl mx-auto mb-6 flex items-center justify-center text-white text-3xl shadow-lg" style={{ background: selectedProvider.color }}>{(selectedProvider as any).icon || selectedProvider.name.charAt(0)}</div>
