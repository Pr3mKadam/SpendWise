...
import { Upload, AlertCircle, CheckCircle2, RefreshCw, FileText } from 'lucide-react';
...
function guessMapping(headers: string[]): ColumnMapping {
  ...
  headers.forEach((h, i) => {
    ...
    else if (['merchant', 'description', 'payee', 'name', 'vendor'].includes(lower))
      mapping[i] = 'merchant';
...
function _parseDate(raw: string): string {
...
function _guessCategory(raw: string): Category {
  const lower = raw.toLowerCase();
  if (['food', 'dining', 'restaurant', 'groceries', 'cafe'].some(k => lower.includes(k)))
    return 'Food';
  ...
  const buildPreview = async () => {
    ...
    try {
      ...
      if (txs && txs.length > 0) {
        ...
        setError(
          'Could not parse transactions. Check that date, merchant and amount columns are mapped correctly.'
        );
      }
    } catch (err) {
      console.error(err);
    ...
    <div className="bg-[var(--surface-card)] rounded-2xl border border-[var(--border)] overflow-hidden">
      <div className="px-5 py-4 border-b border-[var(--border)] flex items-center justify-between">
        ...
        {step !== 'upload' && (
          <button
            onClick={reset}
            className="p-2 rounded-lg hover:bg-[var(--surface-input)] text-[var(--text-muted)] border-none bg-transparent cursor-pointer"
          >
            <RefreshCw size={14} />
          </button>
        )}
      ...
      <div className="p-5">
        {step === 'upload' && (
          ...
          >
            ...
            <Upload
              size={24}
              className={`mx-auto mb-3 ${isDragging ? 'text-[var(--teal)]' : 'text-[var(--text-dim)]'}`}
            />
            <p className="font-inter text-xs font-bold text-[var(--text-primary)]">
              Drop CSV statement
            </p>
        ...
        {step === 'mapping' && (
          <div className="space-y-3">
            <div className="p-2.5 rounded-lg bg-[var(--teal-dim)] border border-[var(--teal-glow)] flex items-center gap-2">
              ...
              <span className="text-[length:var(--fs-overline)] font-bold text-[var(--teal)] truncate">
                {fileName}
              </span>
            </div>
            <div className="max-h-[200px] overflow-y-auto space-y-2 pr-1">
              {headers.map((h, i) => (
                <div
                  key={i}
                  className="flex items-center gap-2 p-2 rounded-lg bg-[var(--surface-input)] border border-[var(--border)]"
                >
                  <div className="flex-1 min-w-0">
                    <p className="text-[length:var(--fs-overline)] font-bold text-[var(--text-primary)] truncate">
                      {h || `Col ${i + 1}`}
                    </p>
                    <p className="text-[length:var(--fs-overline)] text-[var(--text-dim)] truncate">
                      e.g. {rawRows[0]?.[i] || '—'}
                    </p>
                  </div>
                  <select
          ...
          <div className="space-y-3">
            <div className="max-h-[240px] overflow-y-auto border border-[var(--border)] rounded-xl divide-y divide-[var(--border)]">
              {parsed.map((tx, i) => (
                <div key={i} className="p-2.5 flex items-center gap-2">
                  <div className="flex-1 min-w-0">
                    <p className="text-[length:var(--fs-caption)] font-bold text-[var(--text-primary)] truncate">
                      {tx.merchant}
                    </p>
                  ...
                  <p className="text-[length:var(--fs-caption)] font-bold text-[var(--text-primary)]">
                    ₹{tx.amount.toFixed(2)}
                  </p>
                </div>
              ))}
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 py-2.5 rounded-xl bg-[var(--surface-input)] text-[var(--text-secondary)] font-bold text-xs border border-[var(--border)] cursor-pointer"
              >
                Back
              </button>
              <button
                ...
                className="flex-1 py-2.5 rounded-xl bg-[var(--teal)] text-white font-bold text-xs border-none cursor-pointer hover:opacity-90"
              >
                Import All
              </button>
            </div>
        ...
        {step === 'done' && (
          <div className="text-center py-6">
            ...
            <p className="text-[length:var(--fs-overline)] text-[var(--text-muted)] mt-1">
              {parsed.length} transactions added
            </p>
            <button
              onClick={reset}
              className="mt-4 px-6 py-2 rounded-lg bg-[var(--surface-input)] text-[var(--text-primary)] font-bold text-[length:var(--fs-overline)] border border-[var(--border)] cursor-pointer"
            >
              Import Another
            </button>
          </div>
