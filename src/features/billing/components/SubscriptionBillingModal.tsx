...
export function SubscriptionBillingModal({ onClose, onPlanChange }: SubscriptionBillingModalProps) {
  ...
  const handleSubscribe = useCallback(
    async (planName: string) => {
      ...
      localStorage.setItem(
        'spendwise_subscription_period_end',
    ...
    ? [
        {
          ...
          date: new Date(
            Number(localStorage.getItem('spendwise_subscription_start'))
          ).toLocaleDateString(),
          status: 'active',
        },
      ]
  ...
  const currentPeriodEnd = periodEnd ? new Date(Number(periodEnd)).toLocaleDateString() : null;
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      ...
      >
        ...
        {currentPeriodEnd && (
          ...
          >
            <Sparkles size={18} />
            <span className="font-semibold">Current period ends on {currentPeriodEnd}</span>
          </div>
        )}
              ...
              <div
                ...
                style={{
                  background: plan.highlighted
                    ...
                    : '#ffffff',
                  border: plan.highlighted ? '2px solid var(--teal)' : '2px solid #edf2f7',
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr style={{ background: 'var(--surface-input)' }}>
                    <th
                      className="text-left p-3 text-xs font-bold"
