...
function getRecommendations(transactions: Transaction[], savingsRate: number): ProductRecommendation[] {
  const recs: ProductRecommendation[] = [];

  const monthlySpend = transactions.filter(t => t.type === 'debit').reduce((s, t) => s + t.amount, 0) / Math.max(1, 3);
  const travelSpend = transactions.filter(t => t.category === 'Travel' && t.type === 'debit').reduce((s, t) => s + t.amount, 0);
  ...
  const onlineSpend = transactions.filter(t => t.merchant && /amazon|flipkart|myntra/i.test(t.merchant)).length;
  if (onlineSpend >= 3) {
    recs.push({
...
export default function ProductDiscovery({ transactions, savingsRate }: ProductDiscoveryProps) {
  const recommendations = useMemo(() => getRecommendations(transactions, savingsRate), [transactions, savingsRate]);
  ...
  return (
    <div className="card overflow-hidden">
      <div className="px-6 py-5 border-b border-[var(--border)] flex items-center gap-2">
        ...
        <h3 className="font-manrope font-bold text-lg text-[var(--text-primary)]">Personalised for you</h3>
      </div>
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                <h4 className="font-manrope font-bold text-sm text-[var(--text-primary)]">{rec.title}</h4>
                {rec.badge && (
              ...
              <p className="text-xs text-[var(--text-muted)] leading-relaxed mb-3">{rec.description}</p>
