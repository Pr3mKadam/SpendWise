...
export function AllowanceCard() {
  ...
  const [frequency, setFrequency] = useState<'weekly' | 'monthly'>(
    parentalState.allowanceFrequency
  );
      ...
      <div className="rounded-2xl p-4 bg-gradient-to-br from-purple-500/10 to-indigo-500/10 border border-purple-500/20">
        ...
        {parentalState.allowanceAmount > 0 ? (
          <div className="space-y-2">
            <div className="flex items-center justify-between text-xs">
              ...
              <span className="font-bold text-[var(--text-primary)]">
                ₹{parentalState.allowanceAmount}/
                {parentalState.allowanceFrequency === 'weekly' ? 'wk' : 'mo'}
              </span>
            </div>
            ...
            <div className="flex items-center justify-between text-xs">
              ...
              <span className="font-medium text-[var(--text-primary)]">
                {parentalState.lastAllowancePayout
                  ? new Date(parentalState.lastAllowancePayout + 'T00:00:00').toLocaleDateString(
                      'en-IN'
                    )
                  : 'Never'}
              </span>
          ...
          <p className="text-xs text-[var(--text-muted)]">
            Set a weekly or monthly allowance to automate pocket money.
          </p>
        )}
      </div>
    );
